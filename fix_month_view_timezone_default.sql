-- Fix get_month_view to ensure it defaults to America/New_York when timezone is UTC
-- The issue: RPC is returning "UTC" instead of defaulting to America/New_York

-- First, verify the current function definition
SELECT 
  proname,
...
-- Drop and recreate with explicit default handling
...
BEGIN
  ...
  -- CRITICAL: Default to America/New_York if not set, NULL, empty, or explicitly UTC
  -- The helper returns 'UTC' when timezone column doesn't exist or is NULL
  IF v_timezone IS NULL OR v_timezone = '' OR v_timezone = 'UTC' THEN
  ...
  RETURN (
    ...
    events AS (
      SELECT
        ...
        -- Use family timezone (defaults to America/New_York if not set)
    ...
    -- Return the final result with the timezone that was actually used
    SELECT JSONB_BUILD_OBJECT(
      ...
      'timezone', v_timezone,  -- This should be 'America/New_York', not 'UTC'
...
-- Test the function to verify it returns the correct timezone
SELECT 
  get_month_view(
