...
-- Use a single function that accepts both date and timestamp (via type coercion)
...
-- Add comment
COMMENT ON FUNCTION detect_schedule_conflicts(uuid, date, date) IS 'Detects scheduling conflicts between children in a family within a date range. Accepts date parameters (timestamps will be auto-coerced to date)';
