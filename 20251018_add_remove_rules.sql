...
-- =====================================================
-- 8) Explainability Functions
-- =====================================================

-- Explain how a single day's availability was computed (powers "Why" chip)
CREATE OR REPLACE FUNCTION explain_day_availability(
  p_child uuid,
  p_date date,
  p_family uuid
) RETURNS jsonb
...
BEGIN
  -- Get cascade setting
  SELECT COALESCE(specificity_cascade, false) INTO use_cascade
  FROM family_settings WHERE family_id = p_family;

  -- Collect all rules for this day
  WITH base AS (
    SELECT
      r.id,
      r.title,
      ...
      tsrange(
        ...
        (p_date + r.end_time)::timestamp,
        '[)'
      ) AS span
    FROM schedule_rules r
    WHERE r.is_active
      AND p_date BETWEEN r.start_date AND r.end_date
      AND (r.rrule->>'freq') = 'WEEKLY'
      AND (to_char(p_date, 'DY') IN (
        SELECT * FROM jsonb_array_elements_text(COALESCE(r.rrule->'byweekday', '[]'::jsonb))
      ))
      AND (
        (r.scope_type = 'child' AND r.scope_id = p_child) OR
        (r.scope_type = 'family' AND r.scope_id = p_family)
      )
  ),
  ordered AS (
    SELECT *
    FROM base
    ORDER BY
      CASE WHEN use_cascade THEN
        CASE scope_type WHEN 'child' THEN 0 ELSE 1 END
      ELSE 0 END,
      CASE rule_kind WHEN 'off' THEN 0 ELSE 1 END,
      updated_at DESC
  )
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'span', jsonb_build_object(
        'start', to_char(lower(span), 'HH24:MI'),
        'end', to_char(upper(span), 'HH24:MI')
      )
    )
  ) INTO j
  FROM ordered;

  -- Build Add-Remove math
  WITH ordered AS (
    SELECT
      r.id,
      r.rule_kind,
  ...
  SELECT
    range_merge(array_agg(span) FILTER (WHERE rule_kind = 'teach')),
    range_merge(array_agg(span) FILTER (WHERE rule_kind = 'off'))
  INTO teach_spans, off_spans
  FROM ordered;

  teach_spans := COALESCE(teach_spans, '{}');
  off_spans := COALESCE(off_spans, '{}');
  eff := range_subtract(teach_spans, off_spans);

  -- Apply overrides
  SELECT
    COALESCE(array_agg(
      CASE WHEN override_kind IN ('day_off', 'cancel_block', 'early_end', 'late_start')
      THEN tsrange(
        (p_date + COALESCE(start_time, time '00:00'))::timestamp,
        (p_date + COALESCE(end_time, time '23:59'))::timestamp,
        '[)'
      ) END
    ) FILTER (WHERE override_kind IN ('day_off', 'cancel_block', 'early_end', 'late_start')), '{}'),
    COALESCE(array_agg(
      CASE WHEN override_kind = 'extra_block'
      THEN tsrange(
        (p_date + start_time)::timestamp,
        (p_date + end_time)::timestamp,
        '[)'
      ) END
    ) FILTER (WHERE override_kind = 'extra_block'), '{}')
  INTO off_o, teach_o
  FROM schedule_overrides
  WHERE (scope_type = 'child' AND scope_id = p_child)
    AND date = p_date
    AND is_active = true;

  eff := range_subtract(
    range_merge(COALESCE(eff, '{}') || teach_o),
    off_o
  );

  RETURN jsonb_build_object(
    'model', CASE WHEN use_cascade THEN 'add-remove + cascade' ELSE 'add-remove' END,
    'rules', COALESCE(j, '[]'::jsonb),
    'overrides', jsonb_build_object(
      'teach', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'start', to_char(lower(x), 'HH24:MI'),
        'end', to_char(upper(x), 'HH24:MI')
      )), '[]'::jsonb) FROM unnest(teach_o) x),
      'off', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'start', to_char(lower(x), 'HH24:MI'),
        'end', to_char(upper(x), 'HH24:MI')
      )), '[]'::jsonb) FROM unnest(off_o) x)
    ),
    'effective', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'start', to_char(lower(x), 'HH24:MI'),
      'end', to_char(upper(x), 'HH24:MI')
    )), '[]'::jsonb) FROM unnest(eff) x)
  );
END$$;
