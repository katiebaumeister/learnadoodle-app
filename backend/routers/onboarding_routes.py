...
async def ensure_family(
    ...
    try:
        # Only allow for parent role (profile created by handle_new_user with role=parent)
        profile_res = supabase.table("profiles").select("id, role").eq("id", user["id"]).maybe_single().execute()
        ...
        # Create family row (id auto-generated by Supabase)
        family_res = supabase.table("family").insert({}).select("id").execute()
        if not family_res.data or len(family_res.data) == 0:
            raise HTTPException(
