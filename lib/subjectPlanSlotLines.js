...
export async function findAcademicYearPlanForSubject(familyId, subjectId) {
  ...
  const hasSubjectInPlan = (data) => {
    ...
    if (labels.some((l) => String(l?.subject_id ?? '') === sid)) return true;
    const eventDates = Array.isArray(plan.plan_event_dates) ? plan.plan_event_dates : [];
    if (eventDates.some((e) => String(e?.subject_id ?? '') === sid)) return true;
    const slotDates = Array.isArray(plan.plan_slot_dates) ? plan.plan_slot_dates : [];
    if (slotDates.some((e) => String(e?.subject_id ?? '') === sid)) return true;
    const targets = plan?.subject_targets_override;
    if (targets && typeof targets === 'object' && !Array.isArray(targets)) {
      if (Object.prototype.hasOwnProperty.call(targets, sid)) return true;
    }
    return false;
  };
  ...
  for (const row of years) {
    ...
    if (ge || !data?.plan) continue;
    const has = hasSubjectInPlan(data);
