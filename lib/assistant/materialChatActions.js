/**
 * Library materials: queries and archive path via materialsClient.
...
export async function executeArchiveMaterialChat(familyId, materialId) {
  try {
    ...
    return {
      success: true,
      userMessage: 'Removed from your library. You can restore it from Library → trash if needed.',
    };
  } catch (e) {
...
export async function executeCreateLinkMaterialChat(familyId, { title, providerUrl, childId, subjectId }) {
  ...
  try {
    ...
    return {
      success: true,
      userMessage: `Added **${material.title || t}** to your library.`,
