...
export function formatSubjectsListLines(subjects, childrenById) {
  if (!subjects?.length) return ['No subjects yet. Add one from **Subjects** (left sidebar) using **+ New**.'];
...
export function parseAddSubjectIntent(userMessage, children) {
  ...
  const active = (children || []).filter((c) => !c.archived);
  const childNameTokens = active
    .map((c) => displayChildName(c).toLowerCase().trim())
    .filter(Boolean)
  ...
  if (!subjectName) {
    // Handle "add subject for Lilly for Dinosaurs" — child comes first, subject is the second "for ..." clause.
    for (const nm of childNameTokens) {
      const escaped = nm.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const re = new RegExp(`\\b(?:subject|class|course)\\s+for\\s+${escaped}\\s+for\\s+([^,.!?\\n]+)$`, 'i');
      const mChildFirst = userMessage.match(re);
      if (mChildFirst?.[1]) {
        subjectName = mChildFirst[1].trim();
        break;
      }
    }
  }
  ...
  subjectName = subjectName.replace(/^for\s+\w+\s+for\s+/i, '').trim();
  subjectName = subjectName.replace(/^for\s+\w+$/i, '').trim();
...
export async function executeUpdateChildChat(familyId, childId, updates) {
  ...
  if (updates.age != null && updates.age !== '') {
    ...
    if (Number.isFinite(age)) allowed.age = age;
  }
  if (updates.avatar != null) allowed.avatar = String(updates.avatar).trim() || null;
  if (Array.isArray(updates.interests)) allowed.interests = updates.interests;

  const hasChildFields = Object.keys(allowed).length > 0;
  const hasNotes = Object.prototype.hasOwnProperty.call(updates, 'notes');

  if (!hasChildFields && !hasNotes) {
  ...
  if (hasNotes) {
    const notes = updates.notes == null ? null : String(updates.notes).trim() || null;
    const { error: profileError } = await supabase
      .from('child_support_profiles')
      .upsert({ child_id: childId, notes }, { onConflict: 'child_id' });
    if (profileError) return { success: false, error: profileError.message || String(profileError) };
  }
