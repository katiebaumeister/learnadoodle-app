...
/** Longest subject name mentioned in the message (case-insensitive). */
export function findSubjectMentionedInMessage(message, subjects = []) {
  const lower = String(message || '').toLowerCase();
  const sorted = [...(subjects || [])]
    .filter((s) => String(s?.name || s?.title || '').trim().length >= 2)
    .sort((a, b) => (
      String(b.name || b.title || '').length - String(a.name || a.title || '').length
    ));
  for (const subject of sorted) {
    const title = String(subject.name || subject.title || '').trim().toLowerCase();
