...
registerCommand({
  ...
  preview(command) {
    ...
    if (command.attachmentId || command.fileName) {
      rows.push({ label: 'Attachment', value: command.fileName || 'Attached file' });
    } else if (Array.isArray(command.materialIds) && command.materialIds.length) {
  ...
  async execute(command) {
    const { createBulletinPost, uploadBulletinMaterial } = await import('../../services/bulletinClient.js');
    ...
    if (command.attachmentId && !materialIds.length) {
      ...
      try {
        const { data: uploaded, error: uploadError } = await uploadBulletinMaterial({
          familyId: command.householdId,
          file,
          subjectId: command.subjectId || null,
        });
        if (uploadError || !uploaded?.id) {
          holdDoodleAttachment(command.attachmentId, file);
          return {
            ok: false,
            message: uploadError?.message || 'Could not upload attachment.',
            error: 'execution',
          };
        }
        materialIds = [uploaded.id];
      } catch (err) {
        holdDoodleAttachment(command.attachmentId, file);
        return {
          ok: false,
          message: err?.message || 'Could not attach file to announcement.',
          error: 'execution',
        };
      }
    }
    ...
    if (typeof window !== 'undefined' && materialIds[0]) {
    ...
    return {
      ...
      message: materialIds.length
        ? `Posted to ${boardLabel(command)} with attachment.`
        : `Posted to ${boardLabel(command)}.`,
      affectedRecords: [{
      ...
      resultData: { postId: data?.id || null, materialId: materialIds[0] || null },
    };
  },
});
