...
/** Day offs: parents and tutors may create/edit/delete; children may not. */
export function authorizeDayOffMutation(command, ctx, action = 'manage') {
  if (ctx?.userRole === 'child') {
    const verb = action === 'remove' ? 'remove' : action === 'update' ? 'update' : 'add';
    return { ok: false, error: `You do not have permission to ${verb} days off.` };
  }
