...
test('calendar event update/delete intents route correctly', async () => {
  ...
  assert.equal(isEventDeleteIntent('delete field trip for lilly', roster), true);
  assert.equal(isEventDeleteIntent('delete History learning day on July 8'), false);
  assert.equal(isEventDeleteIntent('delete History', roster), false);
  assert.equal(isEventDeleteIntent('delete mental health day', roster), false);
...
test('app guide answers never mention backlog', async () => {
  ...
  assert.match(res.message || '', /can.?t do that function via chat yet/i);
  assert.equal(/app overview/i.test(res.message || ''), false);
});

test('unresolved chat asks point to Settings Feedback', async () => {
  const res = await doodleRespond({
    message: 'blorble the frobnicator',
    context: baseCtx,
    roster,
  });
  assert.equal(res.type, DOODLE_RESPONSE_TYPES.ANSWER);
  assert.match(res.message || '', /can.?t do that function via chat yet/i);
  assert.match(res.message || '', /Settings.+Feedback/i);
  assert.equal(/app overview/i.test(res.message || ''), false);
  assert.ok((res.links || []).some((l) => /feedback/i.test(l.href || '') || /feedback/i.test(l.label || '')));
});
...
test('day off and event delete intents match titles without type keywords', async () => {
  const {
    isDayOffDeleteIntent,
    isEventDeleteIntent,
  } = await import('./intentPreparers.js');

  assert.equal(isDayOffDeleteIntent('delete day off'), true);
  assert.equal(isDayOffDeleteIntent('delete mental health day'), true);
  assert.equal(isDayOffDeleteIntent('remove spring break'), true);
  assert.equal(isDayOffDeleteIntent('delete History learning day'), false);

  assert.equal(isEventDeleteIntent('delete field trip for lilly', roster), true);
  assert.equal(isEventDeleteIntent('delete the field trip event', roster), true);
  assert.equal(isEventDeleteIntent('delete mental health day', roster), false);
  assert.equal(isEventDeleteIntent('delete Lilly', roster), false);
});
...
test('subject delete/grading/syllabus intents cover Subject settings modal', async () => {
  ...
  assert.equal(isSubjectDeleteIntent('delete History', roster), true);
  assert.equal(isSubjectDeleteIntent('delete cinematography', {
    subjects: [{ id: 's9', name: 'cinematography' }],
  }), true);
  assert.equal(isSubjectDeleteIntent('delete the field trip event', roster), false);
  ...
  const deleteByNameOnly = await prepareSubjectDelete('delete History', baseCtx, roster);
  assert.equal(deleteByNameOnly.type, DOODLE_RESPONSE_TYPES.CLARIFICATION);
  assert.equal(deleteByNameOnly.clarification?.field, 'confirmName');
  assert.equal(deleteByNameOnly.clarification?.draft?.subjectId, 's1');
...
test('child update/delete/invite intents cover Edit Family actions', async () => {
  ...
  assert.equal(isChildDeleteIntent('delete Lilly', roster), true);
  assert.equal(isChildDeleteIntent('delete Max', {
    children: [{ id: 'c2', first_name: 'Max' }],
  }), true);
  assert.equal(isChildDeleteIntent('delete History', roster), false);
  ...
  const deleteByNameOnly = await prepareChildDelete('delete Lilly', baseCtx, roster);
  assert.equal(deleteByNameOnly.type, DOODLE_RESPONSE_TYPES.CLARIFICATION);
  assert.equal(deleteByNameOnly.clarification?.field, 'confirmName');
  assert.equal(deleteByNameOnly.clarification?.draft?.childId, 'c1');
