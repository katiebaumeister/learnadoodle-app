...
registerCommand({
  ...
  async execute(command) {
    ...
    try {
      const { seedSubjectGettingStartedBulletinPost } = await import('../../subjectGettingStartedBulletin.js');
      ...
      console.warn('[subject.create] Failed to seed subject bulletin welcome post:', seedErr);
    }
  ...
  async execute(command, ctx) {
    ...
    try {
      ...
      for (const dateYmd of dateList) {
        ...
        try {
          saved = await saveLesson({
            ...
            date,
            // Only stamp a lesson label when creating a real curriculum lesson;
            // otherwise Classwork hides the day as “already linked.”
            lessonLabel: command.createLesson ? lessonTitle : '',
            unitTitle: unitTitle || '',
            curriculumLessonId: command.createLesson ? curriculumLessonId : null,
      ...
      if (typeof window !== 'undefined') {
        ...
        // Drop stale subject-detail preloads so Classwork refetches the new learning day.
        window.dispatchEvent(new CustomEvent('subjectDetailMaterialsStale', {
          detail: {
            familyId: command.householdId,
            subjectIds: [String(command.subjectId)],
          },
        }));
        window.dispatchEvent(new CustomEvent('refreshSubjects'));
