...
registerCommand({
  ...
  authorize(command, ctx, caps = {}) {
    ...
    // Bulk range matches Attendance check modal: parents + tutors (not children).
    if (ctx.userRole === 'child') {
      return { ok: false, error: 'Only parents and tutors can mark a full attendance range.' };
    }
    if (ctx.userRole === 'tutor' && caps.canMarkAttendance === false) {
      return { ok: false, error: 'You do not have permission to mark attendance.' };
    }
    return requireHouseholdMatch(command.householdId, ctx);
  },
  ...
  preview(command) {
    const studentLabel = Array.isArray(command.childNames) && command.childNames.length
      ? command.childNames.join(', ')
      : `${(command.childIds || []).length} selected`;
    const rows = [];
    if (command.termLabel) {
      rows.push({ label: 'Term', value: command.termLabel });
    }
    rows.push(
      {
        label: 'Range',
        value: `${formatDisplayDate(command.startDate)} – ${formatDisplayDate(command.endDate)}`,
      },
      { label: 'Learning days', value: String(command.eventCount || command.eventIds?.length || 0) },
      { label: 'Status', value: 'attended' },
      { label: 'Students', value: studentLabel },
      {
        label: 'Note',
        value: 'Future days and days with no lessons are left untouched',
      },
    );
    return rows;
  },
