...
export async function doodleRespond(input) {
  ...
  const isLessonOrLearningDayCreate =
    (/\b(create|add|schedule|make)\b/.test(lower) && /\b(lesson|learning\s*day)\b/.test(lower))
  ...
  if (isSubjectDeleteIntent(message)) {
  ...
  if (isSubjectUpdateIntent(message, roster)) {
    return trackPreview(await prepareSubjectUpdate(message, ctx, roster));
  }
  ...
  if (isBulletinPostCreateIntent(message)) {
    return trackPreview(await prepareBulletinPostCreate(message, ctx, roster));
  }
  ...
  if (isChildDeleteIntent(message)) {
  ...
  if (
    ...
    return trackPreview(prepareLearningDayCreate(message, ctx, roster, {
      attachments,
      createLesson: /\blesson\b/.test(lower),
    }));
  }
