...
export function isSubjectDeleteIntent(message, roster = {}) {
  const lower = String(message || '').toLowerCase();
  if (!/\b(delete|remove|drop)\b/.test(lower)) return false;
  // Leave non-subject delete flows alone.
  if (/\b(material|materials|library|book|books|workbook|textbook|event|lesson|calendar|appointment|meeting|day\s*off|holiday|break|child|learner|student|assignment|bulletin|message|chat)\b/.test(lower)) {
    return false;
  }
  if (/\bsubjects?\b/.test(lower)) return true;
  const mentioned = findSubjectMentionedInMessage(message, roster.subjects || []);
  return Boolean(mentioned.ok);
}
...
export function isSubjectUpdateIntent(message, roster = {}, options = {}) {
  const lower = String(message || '').toLowerCase();
  if (isSubjectDeleteIntent(message, roster)) return false;
...
export async function prepareAttendanceMarkRange(message, ctx, roster, overrides = {}) {
  ...
  if (ctx.userRole === 'child') {
    return assertDoodleResponse({
      ...
      links: [{ label: 'Open Planner (Year)', href: '/?view=year' }],
    });
  }
...
export function isDayOffDeleteIntent(message) {
  ...
  if (/\b(subject|subjects|child|learner|student|material|materials|assignment|event|appointment|lesson)\b/.test(lower)) {
    return false;
  }
  if (/\b(day\s*off|holiday|break)\b/.test(lower)) return true;
  // Titles like “mental health day” / “teacher work day”
  return /\bday\b/.test(lower);
}
...
export function isChildDeleteIntent(message, roster = {}) {
  const lower = String(message || '').toLowerCase();
  if (!/\b(delete|remove|drop)\b/.test(lower)) return false;
  // Leave non-child delete flows alone.
  if (/\b(material|materials|library|book|books|subject|subjects|event|lesson|calendar|appointment|meeting|day\s*off|holiday|break|assignment|bulletin|message|chat)\b/.test(lower)) {
    return false;
  }
  if (/\b(child|learner|student)\b/.test(lower)) return true;
  const children = roster.children || [];
  for (const child of children) {
    const name = String(child.first_name || child.name || '').trim().toLowerCase();
...
export function resolveFlexibleDay(message, fromDate = new Date()) {
  ...
  if (/^today'?s?$/i.test(raw) || /\btoday'?s?\b/i.test(raw)) return toYmd(fromDate);
  // tomorrow / tomorrow's / tomorrows
  if (/^tomorrow'?s?$/i.test(raw) || /\btomorrow'?s?\b/i.test(raw)) {
    const d = new Date(fromDate);
    d.setDate(d.getDate() + 1);
    return toYmd(d);
  }
  if (/^yesterday'?s?$/i.test(raw) || /\byesterday'?s?\b/i.test(raw)) {
    const d = new Date(fromDate);
    d.setDate(d.getDate() - 1);
    return toYmd(d);
  }
...
export function prepareLearningDayCreate(message, ctx, roster, overrides = {}) {
  ...
  if (!subjectId) {
    const mentioned = findSubjectMentionedInMessage(message, roster.subjects || []);
    if (mentioned.ok) subjectId = String(mentioned.subject.id);
  }
  if (!subjectId) {
    // “for History tomorrow” — stop before date / learner words (spaces used to swallow “tomorrow”).
    const inSubject = message.match(
      /\b(?:for|in)\s+([A-Za-z][A-Za-z0-9&/-]*(?:\s+[A-Za-z][A-Za-z0-9&/-]*){0,4}?)(?=\s+(?:today|tomorrow|yesterday|on|at|from|to|with|using|for|next|this|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b|[.,?!]|$)/i,
    );
    if (inSubject) {
      const candidate = inSubject[1].trim();
      if (!/^(today|tomorrow|yesterday|this|that|me|us)$/i.test(candidate)) {
        const asSubject = resolveSubjectByName(candidate, roster.subjects);
        if (asSubject.clarification) {
          return withClarificationMeta(assertDoodleResponse(asSubject.clarification), {
            intent: 'learning_day.create',
            field: 'subjectId',
            originalMessage: message,
            draft: draftBase,
  ...
  else childIds = childIds.map(String).filter(Boolean);

  // Prefer the subject's assigned students (may be semicolon-joined in child_id).
  // Empty child_id means the subject applies to all household learners.
  if (!childIds.length) {
    const subject = (roster.subjects || []).find((s) => String(s.id) === String(subjectId));
    if (subject) {
      const rawChildId = subject.child_id ?? subject.childIds ?? '';
      const hasExplicitAssignment = rawChildId != null && String(rawChildId).trim() !== '';
      if (hasExplicitAssignment) {
        const fromSubject = String(rawChildId)
          .split(/[;,]/)
          .map((s) => s.trim())
          .filter(Boolean);
        if (fromSubject.length) childIds = fromSubject;
      } else if (Array.isArray(subject.assignedChildren) && subject.assignedChildren.length) {
        childIds = subject.assignedChildren.map(String);
      } else if ((roster.children || []).length) {
        childIds = (roster.children || []).map((c) => String(c.id));
      }
    }
  ...
  if (!childIds.length) {
    // Named learner in the message
    const nameMatch = message.match(/\bfor\s+([A-Za-z][A-Za-z'-]*)\b/i);
    if (nameMatch && !/^(today|tomorrow|this|that|me|us)$/i.test(nameMatch[1])) {
      const resolved = resolveChildByName(nameMatch[1], roster.children);
      if (resolved.clarification) {
        return withClarificationMeta(assertDoodleResponse(resolved.clarification), {
          intent: 'learning_day.create',
...
/**
 * True when the user is amending a confirmable action preview
 * (e.g. “and lesson 1” after Unit 1 was already proposed).
 */
export function isPendingActionRefineIntent(message, pendingAction) {
  if (!pendingAction?.command?.type) return false;
  const lower = String(message || '').toLowerCase().trim();
  if (!lower) return false;
  if (
    /^(cancel|nevermind|never mind|nope|no|confirm|yes|yep|ok|okay|save|do it|go ahead|looks good)$/i
      .test(lower)
  ) {
    return false;
  }
  if (/^(and|also|plus|with)\b/.test(lower)) return true;
  if (/\balso\s+(do|add|include|set)\b/.test(lower)) return true;
  if (
    /\b(add|include|set)\b/.test(lower)
    && /\b(lesson|unit|note|notes|time|minute|attachment|duration)\b/.test(lower)
  ) {
    return true;
  }
  const type = String(pendingAction.command.type);
  if (type === DOODLE_COMMAND_TYPES.LEARNING_DAY_UPDATE || type === 'learning_day.update') {
    const ul = parseUnitLessonFromMessage(message);
    if (ul.unitTitle || ul.lessonTitle) return true;
  }
  return false;
}

/**
 * Merge a follow-up into the pending action and rebuild the preview.
 */
export async function refinePendingAction(message, pendingAction, ctx, roster) {
  const command = pendingAction?.command;
  if (!command?.type) return null;
  const type = String(command.type);

  if (type === DOODLE_COMMAND_TYPES.LEARNING_DAY_UPDATE || type === 'learning_day.update') {
    return prepareLearningDayUpdate(message, ctx, roster, {
  ...
  if (type === DOODLE_COMMAND_TYPES.EVENT_UPDATE || type === 'event.update') {
    return prepareEventUpdate(message, ctx, roster, {
      eventId: command.eventId,
...
/**
 * Parse “unit 1”, “to a new unit 1”, “unit 1 lesson 1” into classwork labels.
 * @returns {{ unitTitle?: string, lessonTitle?: string }}
 */
export function parseUnitLessonFromMessage(message) {
  const text = String(message || '').trim();
  if (!text) return {};
  const out = {};
  const combo = text.match(
    /\bunit\s+(\d+|[A-Za-z][\w\s-]{0,60}?)\s*(?:,?\s+|\s+\/\s+|\s+)\s*lesson\s+(\d+|[A-Za-z][\w\s-]{0,60}?)(?:\b|$)/i,
  );
  if (combo) {
    out.unitTitle = normalizeCurriculumLabel(combo[1], 'Unit');
    out.lessonTitle = normalizeCurriculumLabel(combo[2], 'Lesson');
    return out;
  }
  const unitTo = text.match(/\bunit\s*(?:to|as|:)\s+["']?([^"'\n]{1,120})["']?/i);
  if (unitTo) {
    out.unitTitle = String(unitTo[1]).trim().replace(/[.,;]+$/, '');
  } else {
    const unitRef = text.match(
      /\b(?:to\s+(?:a\s+)?(?:new\s+)?|into\s+(?:a\s+)?(?:new\s+)?|new\s+)?unit\s+(\d+|[A-Za-z][\w\s-]{0,60}?)(?:\b|$)/i,
    );
    if (unitRef) {
      out.unitTitle = normalizeCurriculumLabel(unitRef[1], 'Unit');
    }
  }
  const lessonTo = text.match(/\b(?:lesson|title)\s*(?:to|as|:)\s+["']?([^"'\n]{1,120})["']?/i);
  if (lessonTo) {
    out.lessonTitle = String(lessonTo[1]).trim().replace(/[.,;]+$/, '');
  } else {
    const lessonRef = text.match(/\blesson\s+(\d+)(?:\b|$)/i);
    if (lessonRef) {
      out.lessonTitle = normalizeCurriculumLabel(lessonRef[1], 'Lesson');
    }
  }
  return out;
}

function normalizeCurriculumLabel(raw, prefix) {
  const t = String(raw || '').trim().replace(/[.,;]+$/, '');
  if (!t) return '';
  if (/^\d+$/.test(t)) return `${prefix} ${t}`;
  const re = new RegExp(`^${prefix}\\s+`, 'i');
  if (re.test(t)) return t.replace(/^\w/, (c) => c.toUpperCase());
  return t;
}
...
async function resolveLearningDayTarget(message, ctx, roster, overrides = {}) {
  ...
  try {
    ...
    // “tomorrow’s learning day” with 2+ days that day → always ask which one
    // (title scoring alone prefers “…learning day” titles and skips the question).
    if (dateFiltered && pool.length > 1) {
      return { ok: false, candidates: pool.slice(0, 8), reason: 'ambiguous' };
    }
    if (pool.length === 1) {
      return { ok: true, event: pool[0] };
    }
...
export async function prepareLearningDayUpdate(message, ctx, roster, overrides = {}) {
  ...
  // Skip when “to …” is clearly a unit/lesson destination (not a calendar day).
  const toDateMatch = message.match(/\bto\s+(today|tomorrow|yesterday|next\s+\w+|\w+day|\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}(?:\/\d{2,4})?)\b/i);
  if (!patch.date && toDateMatch && !/\bto\s+(?:a\s+)?(?:new\s+)?unit\b/i.test(message)) {
  ...
  const unitLesson = parseUnitLessonFromMessage(message);
  if (!patch.unlinkLesson) {
    if (!patch.unitTitle && unitLesson.unitTitle) patch.unitTitle = unitLesson.unitTitle;
    if (!patch.lessonTitle && unitLesson.lessonTitle) patch.lessonTitle = unitLesson.lessonTitle;
  }
  // “and lesson 1” / “also lesson 1” while refining a pending preview
  if (!patch.unlinkLesson && !patch.lessonTitle) {
    const andLesson = message.match(/\b(?:and|also|plus|with)\s+lesson\s+(\d+|[A-Za-z][\w\s-]{0,40}?)(?:\b|$)/i);
    if (andLesson) {
      patch.lessonTitle = normalizeCurriculumLabel(andLesson[1], 'Lesson');
    }
  }
  ...
  if (!Object.keys(patch).length) {
    return withClarificationMeta(assertDoodleResponse({
      type: DOODLE_RESPONSE_TYPES.CLARIFICATION,
      message: 'What should I change on this learning day? Examples: date/time, duration, session notes, unit, lesson, or attachment.',
    }), {
      intent: 'learning_day.update',
      field: 'patchDetail',
      originalMessage: message,
...
export function parseMoveTargetDay(message, fromDate = new Date()) {
  ...
  if (/\btoday'?s?\b/.test(lower)) return base;
  if (/\btomorrow'?s?\b/.test(lower)) {
    const d = new Date(base);
    d.setDate(d.getDate() + 1);
    return d;
  }
  if (/\byesterday'?s?\b/.test(lower)) {
    const d = new Date(base);
    d.setDate(d.getDate() - 1);
    return d;
  }
...
export function continueIntentClarification(pendingClarification, message, option, ctx, roster) {
  ...
  if (field === 'subjectId') {
    ...
    const rawId = String(option?.value || value || '').trim();
    // Free-text follow-ups (e.g. “also do lesson 1”) must not become event IDs.
    const looksLikeId = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(rawId)
      || (/^[0-9a-f-]{16,}$/i.test(rawId) && !/\s/.test(rawId));
    if (!looksLikeId && !option?.value) {
      if (pendingClarification.intent === 'learning_day.update' && draft.eventId) {
        return prepareLearningDayUpdate(String(message || value || ''), ctx, roster, {
          ...draft,
          eventId: draft.eventId,
          eventTitle: draft.eventTitle || draft.itemTitle,
        });
      }
      return null;
    }
    draft.itemId = String(option?.value || rawId);
    draft.eventId = draft.itemId;
    draft.itemTitle = option?.label || draft.itemTitle;
    draft.eventTitle = draft.itemTitle;
  } else if (field === 'patchDetail') {
    // Free-text follow-up (e.g. “unit 1 lesson 1”) after “what should I change?”
    return prepareLearningDayUpdate(String(message || value || ''), ctx, roster, {
      ...draft,
      eventId: draft.eventId || draft.itemId,
      eventTitle: draft.eventTitle || draft.itemTitle,
    });
...
export function isEventDeleteIntent(message, roster = {}) {
  const lower = String(message || '').toLowerCase();
  if (!/\b(delete|remove|cancel)\b/.test(lower)) return false;
  if (/\blearning\s*day\b/.test(lower)) return false;
  if (/\b(subject|subjects|child|learner|student|material|materials|assignment|bulletin)\b/.test(lower)) {
    return false;
  }
  if (isDayOffDeleteIntent(message)) return false;
  // “delete Lilly” / “delete History” belong to child/subject flows — not events that mention them.
  if (isBareRosterEntityDelete(message, roster)) return false;
  if (/\b(event|appointment|field\s*trip|calendar)\b/.test(lower)) return true;
  if (/\b(from\s+(the\s+)?calendar|from\s+planner)\b/.test(lower)) return true;
  // “delete dentist” / “remove museum visit” without saying “event”
  const rest = lower
    .replace(/\b(delete|remove|cancel|the|a|an|please|from|planner|calendar)\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  return rest.length >= 2;
}

/** True when the message is only delete/remove + a roster child or subject name. */
function isBareRosterEntityDelete(message, roster = {}) {
  const m = String(message || '').trim().match(
    /^(?:please\s+)?(?:delete|remove|drop)\s+(?:the\s+)?(.+?)\s*[.?!]*$/i,
  );
  if (!m) return false;
  const name = String(m[1] || '').trim().toLowerCase();
  if (!name) return false;
  for (const child of roster.children || []) {
    const n = String(child.first_name || child.name || '').trim().toLowerCase();
    if (n && name === n) return true;
  }
  for (const subject of roster.subjects || []) {
    const n = String(subject.name || subject.title || '').trim().toLowerCase();
    if (n && name === n) return true;
  }
  return false;
}

export function isEventUpdateIntent(message, roster = {}) {
  const lower = String(message || '').toLowerCase();
  if (isEventDeleteIntent(message, roster) || isLearningDayUpdateIntent(message) || isLearningDayDeleteIntent(message)) {
    return false;
  }
