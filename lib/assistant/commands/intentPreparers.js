...
export function isSubjectUpdateIntent(message, roster = {}) {
  const lower = String(message || '').toLowerCase();
  ...
  if (!mentioned.ok && !hasSubjectWord) return false;
...
function parseSubjectSchoolYear(message, fallback = null) {
  const m = String(message || '').match(/\b(20\d{2})\s*\/\s*(\d{2})\b/);
  if (m) return `${m[1]}/${m[2]}`;
  return fallback;
}
...
export function isSchoolYearSettingsIntent(message) {
  ...
  return /\b(school\s*year|learning\s*hours|hours?\s*per\s*day|target\s*days|learning\s*days|weekdays?|term\s+dates?|fall\s+term|spring\s+term|summer\s+(?:term|range)|attendance\s*(?:goal|tracking|mode)|tracking\s*mode|total\s+class\s+days|per\s+subject|school\s*hours|default\s+learning)\b/.test(lower);
}
...
export function prepareSchoolYearUpdate(message, ctx, overrides = {}) {
  ...
  const parseTerm = (termKey, patterns) => {
    for (const re of patterns) {
      ...
      if (!m) continue;
      const start = parseNamedCalendarDate(m[1], new Date()) || resolveFlexibleDay(m[1], new Date());
      const end = parseNamedCalendarDate(m[2], new Date()) || resolveFlexibleDay(m[2], new Date());
  ...
  if (yearRange && !/fall|spring|summer/.test(yearRange[0].toLowerCase())) {
    const start = parseNamedCalendarDate(yearRange[1], new Date()) || resolveFlexibleDay(yearRange[1], new Date());
    const end = parseNamedCalendarDate(yearRange[2], new Date()) || resolveFlexibleDay(yearRange[2], new Date());
  ...
  if (!Object.keys(patch).length) {
    return assertDoodleResponse({
      type: DOODLE_RESPONSE_TYPES.CLARIFICATION,
      message: 'What should I change in School Year Settings? Examples: “set learning days to Mon Tue Wed”, “set school hours from 9am to 3pm”, “set attendance goal to 80 days”, or “set fall term from Aug 7 to Dec 19”.',
    });
  }
  ...
  return assertDoodleResponse({
    ...
    command,
    preview: handler.preview(command),
    warnings: ['Same fields as School Year Settings — confirm to save.'],
...
export async function prepareSubjectUpdate(message, ctx, roster, overrides = {}) {
  ...
  const schoolYear = parseSubjectSchoolYear(message);
  ...
  return assertDoodleResponse({
    ...
    preview: handler.preview(command),
    warnings: schedule
      ? ['This updates the subject’s recurring schedule on the planner (same as Subject settings → Save).']
      : undefined,
...
export async function prepareLearningDayDelete(message, ctx, roster, overrides = {}) {
  ...
  return assertDoodleResponse({
    ...
    command,
    preview: handler.preview(command),
    warnings: ['This removes the learning day from the planner (same as Delete learning day in the modal).'],
...
export async function prepareLearningDayUpdate(message, ctx, roster, overrides = {}) {
  ...
  return assertDoodleResponse({
    ...
    command,
    preview: handler.preview(command),
    warnings: ['Same fields as the Learning day modal — confirm to save.'],
...
export async function prepareEventDelete(message, ctx, roster, overrides = {}) {
  ...
  return assertDoodleResponse({
    ...
    command,
    preview: handler.preview(command),
    warnings: ['Same as Delete event in the Calendar Event modal.'],
...
export async function prepareEventUpdate(message, ctx, roster, overrides = {}) {
  ...
  try {
    ...
    return assertDoodleResponse({
      ...
      command,
      preview: handler.preview(command),
      warnings: ['Same fields as the Calendar Event modal — confirm to save.'],
      confirmationLabel: 'Save changes',
