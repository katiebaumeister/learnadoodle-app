 ...
 * @param {{ fetch?: string, openTaskModal?: object, createEventInBackground?: object, pendingCommit?: object, disambiguation?: object } | null} legacy
 */
export function createAssistantResponse(type, content, tool = null, meta = null, legacy = null) {
  ...
  if (legacy?.pendingCommit) response.pendingCommit = legacy.pendingCommit;
  if (legacy?.disambiguation) response.disambiguation = legacy.disambiguation;
  return response;
}

/** @param {object | null} response */
export function getDisambiguation(response) {
  return response?.disambiguation ?? null;
}
