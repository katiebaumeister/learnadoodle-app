...
import { generateFamilyICS, generateChildICS } from './icsService.js';
  ...
  app.get('/api/search', async (req, res) => {
    try {
      ...
      const { data: childrenData } = await supabase
        .from('children')
        // Some databases don't have a standalone `name` column on children,
        // so select only fields that are guaranteed to exist.
      ...
      const { data: uploads } = await supabase
        .from('uploads')
        .select('id, storage_path, title, filename, kind, child_id')
        .eq('family_id', familyId)
        .or(`title.ilike.%${q}%,storage_path.ilike.%${q}%,filename.ilike.%${q}%`)
        .limit(5);
      ...
      if (uploads) {
        uploads.forEach((u) => {
          ...
          const displayTitle = u.title || u.filename || (u.storage_path ? u.storage_path.split('/').pop() : 'Untitled');
          results.push({
            id: `upload-${u.id}`,
            type: 'document',
            title: displayTitle,
