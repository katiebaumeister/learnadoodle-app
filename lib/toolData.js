 ...
 * @param {string[]} [params.children] - Array of child IDs (deprecated - use session instead)
 * @param {Object} [params.session] - Session context for role-based filtering
...
export async function fetchTasks(params) {
  try {
    ...
    // Apply role-based child filter if session provided
    if (params.session) {
      const { applyChildFilter } = await import('./queryFilters');
      query = applyChildFilter(query, params.session, 'child_id');
    } else if (params.children && params.children.length > 0) {
      // Legacy: Apply child filter from params
