...
function normalizeGuideLanguage(rawText) {
  ...
  text = text.replace(/\bSubjects\s*>\s*Subjects\b/gi, 'Subjects');
  text = text.replace(/,\s*Learning\s*\(/gi, ', Subjects (');
  text = text.replace(/\bopen\s+Learning\b/gi, 'open Subjects');
  text = text.replace(/\bplanner attendance\b/gi, 'Subjects > Progress');
  text = text.replace(/\battendance page\b/gi, 'Subjects > Progress');
  text = text.replace(/\bplanner blocks?\b/gi, 'scheduled events');
  text = text.replace(/\btime slots?\b/gi, 'event times');
  text = text.replace(/\bslots?\b/gi, 'event times');
  text = text.replace(/\bfix gap can schedule lessons\b/gi, 'Fix gap can add recurring events');
  text = text.replace(/\badjust the plan around that\b/gi, 'adjust events around that');
  return text;
}
...
function isGuideHitRelevant(userMessage, hitContent) {
  const q = String(userMessage || '').toLowerCase();
  const h = String(hitContent || '').toLowerCase();
  if (!q || !h) return false;

  const stop = new Set([
    'how', 'what', 'when', 'where', 'why', 'can', 'could', 'would', 'should', 'please',
    'the', 'this', 'that', 'with', 'from', 'into', 'over', 'under', 'your', 'our',
    'for', 'and', 'or', 'but', 'then', 'than', 'just', 'have', 'has', 'had',
  ]);
  const queryTokens = q
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
  ...
  const eventIntent = /(event|events|planner|calendar|recurr|repeat|weekly|monthly|year)/i.test(q);
  if (eventIntent) {
    const guideIsEventRelated = /(event|planner|calendar|repeat|recurr|weekly|monthly|\+ new)/i.test(h);
    if (!guideIsEventRelated) return false;
  }
  return matched >= 1;
}

function getDeterministicHowToReply(userMessage) {
  const msg = String(userMessage || '').toLowerCase();
  const asksRecurringEvents =
    /(event|events|lesson|class|appointment|planner|calendar)/.test(msg) &&
    /(repeat|recurr|weekly|monthly|year|school year|all year|over a year)/.test(msg);
  if (!asksRecurringEvents) return null;

  return 'Open Planner, click + New, create the event, then turn on Repeat. Choose Weekly or Monthly, pick the days, and under Ends choose On date (set the school-year end) or Term end if available. Save once and it will create the repeating series. To send it to a student, open that event, tap Edit event, then use Send to student.';
}

function getDeterministicAttendanceLocationReply(userMessage) {
  const msg = String(userMessage || '').toLowerCase();
  const asksAttendanceLocation =
    /(where|how|track|tracking|see|view|open|check|find)/.test(msg) &&
    /(attendance|attend)/.test(msg);
  if (!asksAttendanceLocation) return null;
  return 'Attendance is in Subjects now. Use Subjects > Progress for child-level attendance, and open a subject in Subjects to update attendance by subject.';
}

function getDeterministicScheduleGapReply(userMessage) {
  const msg = String(userMessage || '').toLowerCase();
  const asksAboutGaps =
    /(gap|fix gap|behind|ahead|target|projected)/.test(msg) &&
    /(schedule|subjects|subject|year|attendance|events?)/.test(msg);
  const asksAboutSchedulePage =
    /(schedule page|subjects schedule|year targets|subject days|how.*schedule|what.*schedule)/.test(msg);
  if (!asksAboutGaps && !asksAboutSchedulePage) return null;
  return 'Subjects > Schedule shows Subject Days and Year Targets. Gap is target minus projected (attended + upcoming). Fix gap adjusts future calendar events by adding or removing events/recurring events to move projected toward target. To assign work, open an event, choose Edit event, and tap Send to student.';
}

function getDeterministicSettingsProfileReply(userMessage) {
  const msg = String(userMessage || '').toLowerCase();
  const asksDeleteAccount = /(delete|remove|close)\s+(my\s+)?account/.test(msg);
  const asksSetTerm = /(set|change|edit|update)\s+(the\s+)?term/.test(msg) || /term\s+settings?/.test(msg);
  if (!asksDeleteAccount && !asksSetTerm) return null;
  if (asksDeleteAccount) return 'Open Settings > Profile to manage account actions, including account deletion options.';
  return 'Open Settings > Profile to update term settings.';
}

function getDeterministicUnsupportedFeatureReply(userMessage) {
  const msg = String(userMessage || '').toLowerCase();
  const asksRestoreDeletedEvents =
    /(restore|recover|undo)/.test(msg) &&
    /(deleted?\s+events?|events?\s+deleted|trash)/.test(msg);
  const asksPerSubjectLearningDaysChange =
    /(change|edit|update|set)/.test(msg) &&
    /(learning\s+days?|saved\s+learning\s+days?)/.test(msg) &&
    /(by\s+subject|per\s+subject|for\s+each\s+subject)/.test(msg);
  if (!asksRestoreDeletedEvents && !asksPerSubjectLearningDaysChange) return null;
  return "That function isn't available yet. Please open Settings > Feedback to share this idea with the team.";
}
...
const suggestCourses = async (context, subjectName, approach = null, meta = null) => {
  try {
    ...
    if (approach === 'live-class') courses = [`Outschool ${subjectName} – 16 wks, ~$280`, `Time4Learning ${subjectName} – 18 wks, ~$200`, `K12 ${subjectName} – 20 wks, ~$350`];
      ...
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, `I'll create a custom ${subjectName} path for you. Let me work on that...`, null, meta, { fetch: 'custom-plan' });
    }
...
function tryFastAnswerFromLocalGuide(userMessage, meta, startMs) {
  ...
  const relevantHits = localHits.filter((h) => isGuideHitRelevant(userMessage, h?.content || ''));
...
async function runGroundedOpenAIReply(
  ...
  if (!hasUsableClientOpenAIKey() || disableClientLlmForSession) {
    const unsupportedReply = getDeterministicUnsupportedFeatureReply(userMessage);
    if (unsupportedReply) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, unsupportedReply, null, meta);
    }
    const settingsProfileReply = getDeterministicSettingsProfileReply(userMessage);
    if (settingsProfileReply) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, settingsProfileReply, null, meta, {
        fetch: 'navigate_family',
      });
    }
    const scheduleGapReply = getDeterministicScheduleGapReply(userMessage);
    if (scheduleGapReply) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, scheduleGapReply, null, meta);
    }
    const deterministic = getDeterministicHowToReply(userMessage);
    if (deterministic) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, deterministic, null, meta);
    }
    const attendanceLocation = getDeterministicAttendanceLocationReply(userMessage);
    if (attendanceLocation) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, attendanceLocation, null, meta);
    }
    const localHits = searchLocalAppGuide(userMessage, { limit: 3 });
    const relevantHits = localHits.filter((h) => isGuideHitRelevant(userMessage, h?.content || ''));
    const best = relevantHits?.[0]?.content ? String(relevantHits[0].content).trim() : '';
    const friendlyBest = best ? toFriendlyPlainText(best, { maxChars: 560, maxSentences: 4 }) : '';
    const fallbackText = best
      ? `Answer: ${friendlyBest}`
      : 'I can help with planner actions, events, attendance, subjects, and materials. Attendance is in Subjects > Progress (child) and subject details (by subject).';
...
- Use event language: describe scheduling as events/recurring events. Avoid "plan" and "slots" wording.
...
- Attendance lives in Subjects now (Subjects > Progress for child-level, subject details for by-subject), not in Planner.
...
export const processDoodleMessage = async (userMessage, familyId, conversationId = null, options = {}) => {
  ...
  try {
    ...
    const deterministicUnsupported = getDeterministicUnsupportedFeatureReply(userMessage);
    if (deterministicUnsupported) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, deterministicUnsupported, null, {
        intent: 'direct_answer',
        confidence: 0.96,
      });
    }
    const deterministicSettingsProfile = getDeterministicSettingsProfileReply(userMessage);
    if (deterministicSettingsProfile) {
      return createAssistantResponse(
        RESPONSE_TYPES.MESSAGE,
        deterministicSettingsProfile,
        null,
        { intent: 'navigate', confidence: 0.96 },
        { fetch: 'navigate_family' }
      );
    }
    const deterministicScheduleGap = getDeterministicScheduleGapReply(userMessage);
    if (deterministicScheduleGap) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, deterministicScheduleGap, null, {
        intent: 'direct_answer',
        confidence: 0.95,
      });
    }
    const deterministicAttendanceLocation = getDeterministicAttendanceLocationReply(userMessage);
    if (deterministicAttendanceLocation) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, deterministicAttendanceLocation, null, {
        intent: 'direct_answer',
        confidence: 0.95,
      });
    }
    const deterministicHowTo = getDeterministicHowToReply(userMessage);
    if (deterministicHowTo) {
      return createAssistantResponse(RESPONSE_TYPES.MESSAGE, deterministicHowTo, null, {
        intent: 'direct_answer',
        confidence: 0.95,
      });
    }
    ...
    switch (intent) {
      ...
      case 'navigate': {
        ...
        let text = '';
        if (/(delete|remove|close)\s+(my\s+)?account/.test(msg)) {
          fetchTarget = 'navigate_family';
          text = 'Opening Settings > Profile for account actions.';
        } else if (/(set|change|edit|update)\s+(the\s+)?term/.test(msg) || /term\s+settings?/.test(msg)) {
          fetchTarget = 'navigate_family';
          text = 'Opening Settings > Profile for term settings.';
        } else if (/feedback|feature request|leave (a )?(comment|note)|send (a )?suggestion/i.test(msg)) {
          ...
          fetchTarget = 'navigate_subjects_progress';
          text = 'Opening Subjects > Progress for attendance.';
 ...
 * userMessage is a natural-language summary for the UI (e.g. "I've added the activity for Emma tomorrow.").
 */
  ...
  try {
    switch (tool) {
      case 'add_activity': {
        ...
        const userMessage = childName ? `I've added "${toolParams.name}" for ${childName}.` : `I've added the activity "${toolParams.name}".`;
      ...
      case 'queue_reschedule': {
        ...
        const userMessage = `I've queued your reschedule for ${toolParams.calendar_date}. I'll help adjust events around that.`;
        logAssistantEvent({ tool_called: tool, execution_result: 'success', latency_ms: Date.now() - startMs });
