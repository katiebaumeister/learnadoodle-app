  ...
  try {
    ...
    switch (intent.intent) {
      ...
      case 'progress_summary': {
        ...
        const mentionedChild = context.children.find(c =>
          msgLower.includes((c.first_name || '').toLowerCase())
        );
        ...
        try {
          ...
          if (!subjects || subjects.length === 0) {
            return {
              message: `${child.first_name} doesn't have any subjects with progress data yet.`,
              tool: null,
              params: null,
              fetch: null
            };
          ...
          const subjectNameMatch = userMessage.match(/(?:progress\s+on|for)\s+(\w+)/i) ||
            userMessage.match(/(\w+)\s+progress/i);
          ...
          if (subjectKeyword) {
            const match = subjects.find(s => {
              ...
              return subjectMatchesKeyword(name, subjectKeyword);
            });
            if (match) {
              const name = match.name || match.subject_name || 'Subject';
              const pct = match.progressPercent != null && !Number.isNaN(match.progressPercent)
                ? Math.round(match.progressPercent)
                : null;
              const line = pct != null
                ? `${name}: ${pct}% progress`
                : `${name}: no progress data yet`;
              return {
                message: `${child.first_name}'s progress — ${line}.`,
                tool: null,
                params: null,
                fetch: null
              };
          ...
          return {
            message: `${child.first_name}'s progress:\n${summary}`,
            tool: null,
            params: null,
            fetch: null
          };
          ...
          return {
            message: "I couldn't load progress data right now. Please try again.",
            tool: null,
            params: null,
            fetch: null,
            debug: err?.message
