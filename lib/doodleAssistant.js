...
async function handleRenameSubjectIntent(userMessage, familyId, context, meta, startMs) {
  ...
  if (!subjects.length) {
    return createAssistantResponse(RESPONSE_TYPES.MESSAGE, 'You have no subjects yet — add one under **Learning > Subjects** first.', null, meta);
  }
  const children = context.children || [];
  const map = new Map(children.map((c) => [String(c.id), c]));
...
- For "where is…" or "how do I…" questions, follow the App guide below. Name the sidebar or area (Home, Planner, Learning, Materials, Settings) and a subsection when useful (e.g. Settings > Feedback).
    ...
    switch (intent) {
      ...
      case 'navigate': {
        ...
        if (/feedback|feature request|leave (a )?(comment|note)|send (a )?suggestion/i.test(msg)) {
          ...
          text = 'Opening Learning > Subjects.';
