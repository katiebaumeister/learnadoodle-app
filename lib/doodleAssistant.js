...
async function handleCheckAttendanceIntent(userMessage, familyId, context, meta, startMs) {
  ...
  if (!c) {
    return createAssistantResponse(RESPONSE_TYPES.MESSAGE, 'Add a child in Family settings first, then I can summarize attendance.', null, meta);
  }
...
function buildDeleteMaterialPendingResponse(material, familyId, meta) {
  ...
  const body = `I'll remove this from your library:\n\n${summarizeDeleteMaterialProposal(snap)}\n\nTap **Remove from library** to confirm (you can restore from Library trash).`;
  return createAssistantResponse(RESPONSE_TYPES.MESSAGE, body, null, meta, {
    pendingCommit: {
...
function buildAddMaterialLinkPendingResponse(parsed, familyId, meta) {
  const body = `I'll add this link to your library:\n\n${summarizeAddMaterialLinkProposal({
    title: parsed.title,
    providerUrl: parsed.providerUrl,
    childName: parsed.childName,
    subjectName: parsed.subjectName,
  })}\n\nTap **Add to library** to confirm.`;
  return createAssistantResponse(RESPONSE_TYPES.MESSAGE, body, null, meta, {
    pendingCommit: {
...
async function handleListMaterialsIntent(userMessage, familyId, meta, startMs) {
  ...
  if (error) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      "I couldn't load your library right now. Open **Library** from the sidebar.",
      null,
      meta
    );
  }
  if (!materials.length) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      'Your materials library is empty. Add files from **Library**, attach them to lessons, or paste a **https://…** link in chat and say **add to library**.',
      null,
      meta
    );
  }
...
async function handleDeleteMaterialIntent(userMessage, familyId, meta, startMs) {
  ...
  if (error) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      "I couldn't load your library. Try again or remove the item from **Library**.",
      null,
      meta
    );
  }
  if (!materials.length) {
    return createAssistantResponse(RESPONSE_TYPES.MESSAGE, 'Your library is empty — nothing to delete.', null, meta);
  }
  ...
  if (!resolved.ok) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      "I couldn't find that material. Open **Library** or name it more specifically (quotes help).",
      null,
      meta
    );
  }
...
async function handleRenameMaterialIntent(userMessage, familyId, meta, startMs) {
  ...
  if (!parsed) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      'Say e.g. **rename "Old title" to "New title"** or **rename workbook in library to Algebra workbook**.',
      null,
      meta
    );
  }
  ...
  if (error) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      "I couldn't load your library. Try again or rename the item in **Library**.",
      null,
      meta
    );
  }
  if (!materials.length) {
    return createAssistantResponse(RESPONSE_TYPES.MESSAGE, 'Your library is empty — nothing to rename.', null, meta);
  }
  ...
  if (!resolved.ok) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      "I couldn't find that material. Open **Library** or name the current title more specifically (quotes help).",
      null,
      meta
    );
  }
...
async function handleAddMaterialIntent(userMessage, familyId, context, meta, startMs) {
  ...
  if (!parsed) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      'Paste a **link** (https://…) and say e.g. **add this to our library** or **save material "Algebra notes" https://…**. To upload a file, open **Library** → **Add material**.',
      null,
      meta
    );
  }
  if (parsed.kind === 'need_url') {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      'I need a **https://…** link to save from chat. Paste the URL, or open **Library** → **Add material** to upload a file.',
      null,
      meta
    );
  }
...
async function handleRenameSubjectIntent(userMessage, familyId, context, meta, startMs) {
  ...
  if (!subjects.length) {
    return createAssistantResponse(RESPONSE_TYPES.MESSAGE, 'You have no subjects yet — add one under **Subjects** first.', null, meta);
  }
  const children = context.children || [];
  const map = new Map(children.map((c) => [String(c.id), c]));
...
async function handleDeleteChildPermanentIntent(userMessage, context, meta, familyId, startMs) {
  ...
  const dn = childDisplayNameForRoster(c);
  if (!confirmName) {
    return createAssistantResponse(RESPONSE_TYPES.MESSAGE, "This profile doesn't have a first name set; use **Family** to remove them.", null, meta);
  }
...
async function handleTodaysScheduleIntent(userMessage, familyId, meta, startMs) {
  ...
  if (list.length === 0) {
    ...
    return createAssistantResponse(
      RESPONSE_TYPES.MESSAGE,
      `Nothing is on your family calendar for **${label}**. Use **Planner** → **+ New** to add something.`,
      null,
      meta
    );
  }
...
function tryFastAnswerFromLocalGuide(userMessage, meta, startMs) {
  ...
  const parts = localHits.slice(0, 2).map((h) => (h.content || '').trim()).filter(Boolean);
  if (!parts.length) return null;
  const body = parts.join('\n\n').slice(0, 4000);
  const text = `Here's what the in-app guide says:\n\n${body}`;
  logAssistantEvent({
...
- For "where is…" or "how do I…" questions, follow the App guide below. Name the sidebar or area (Home, Planner, Subjects, Library, Family) and a subsection when useful (e.g. Family → Feedback).
- If the App guide does not mention a feature, say you are not sure it exists yet and tell them to use **Family → Feedback** to request it or describe what they need. Do not invent features.
- Never promise dates for unreleased work; feedback goes through Family → Feedback.
  ...
  try {
    ...
    if (disambigPick?.intent === 'rename_material') {
      ...
      if (!nt) {
        return createAssistantResponse(
          RESPONSE_TYPES.MESSAGE,
          'Ask again with **rename "current title" to "new title"** (or include library / workbook in your message).',
          null,
    ...
    switch (intent) {
      ...
      case 'navigate': {
        ...
        if (/feedback|feature request|leave (a )?(comment|note)|send (a )?suggestion/i.test(msg)) {
          ...
          text = 'Opening Family → Feedback so you can send a note to the team.';
