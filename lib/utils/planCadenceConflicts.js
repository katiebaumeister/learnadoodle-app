...
function isEventSkippedForOverlap(ev) {
  ...
  // Plan-generated slots are intentionally marked flexible; they still must count for cadence conflicts.
  if (ev.is_flexible && ev.generated_by !== 'plan_year') return true;
  // Recurring events should still participate in overlap checks when they have concrete times.
  // Only skip recurrence templates that lack usable timestamps.
  if (ev.recurrence_rule && (!ev.start_ts || !ev.end_ts)) return true;
  return false;
}

/**
 * Skip plan_year events that belong to the same academic year AND one of the subjects
 * currently being edited by cadence blocks (those will be replaced on apply).
 */
export function shouldSkipEventForPlanConflict(ev, academicYearId, activeSubjectIds = null) {
  if (!ev || !academicYearId) return false;
  if (
    ev.generated_by === 'plan_year' &&
    String(ev.academic_year_id || '') === String(academicYearId)
  ) {
    if (!Array.isArray(activeSubjectIds) || activeSubjectIds.length === 0) return true;
    const sid = ev.subject_id != null ? String(ev.subject_id) : null;
    if (!sid) return false;
    if (!activeSubjectIds.map(String).includes(sid)) return false;
    return true;
  }
  return false;
}
...
export function analyzePlanCadenceConflicts({
  ...
  const activeSubjectIds = [...new Set((blocks || []).map((b) => b?.subject_id).filter(Boolean))];
  ...
  events.forEach((ev) => {
    ...
    if (shouldSkipEventForPlanConflict(ev, academicYearId, activeSubjectIds)) return;
    slots.forEach((slot) => {
