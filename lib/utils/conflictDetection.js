...
function isSkippedForOverlapRow(ev) {
  ...
  if (!Number.isNaN(start.getTime()) && !Number.isNaN(end.getTime())) {
    ...
    // Legacy "no explicit time" events were sometimes persisted as full-day windows.
    // Treat these as flexible for chip conflict warnings.
...
export function detectConflicts(movedEvent, allEvents) {
  ...
  if (isSkippedForOverlapRow(movedEvent)) return 0;
...
export function findFirstConflictEvent(movedEvent, allEvents, options = {}) {
  ...
  if (isSkippedForOverlapRow(movedEvent)) return null;
