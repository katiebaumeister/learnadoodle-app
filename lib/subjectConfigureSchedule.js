...
import { applyToCalendar, getAcademicYear, getAcademicYearExclusions } from './services/academicYearClient';
...
export function buildInitialScheduleForm({ subject, planData, academicYearId }) {
  const block = getSubjectPlanBlocksForSubject(planData, subject?.id)[0] || null;
  const weekdays = parseWeekdaysFromBlock(block);
  const startTime = normalizeHm(block?.start_time, '09:00');
  const durationMinutes = durationMinutesFromTimes(startTime, block?.end_time);
  const labelRange = defaultSchoolYearRange(subject);
  const plan = planData?.plan || {};
  const startDateStr = String(plan?.start_date || planData?.start_date || labelRange?.start_date || '').slice(0, 10);
  const endDateStr = String(plan?.end_date || planData?.end_date || labelRange?.end_date || '').slice(0, 10);

  return {
    weekdays: weekdays.length > 0 ? weekdays : [1, 3, 5],
    startTime,
    durationMinutes,
    startDate: DATE_KEY_RE.test(startDateStr) ? new Date(`${startDateStr}T12:00:00`) : new Date(`${labelRange.start_date}T12:00:00`),
    endDate: DATE_KEY_RE.test(endDateStr) ? new Date(`${endDateStr}T12:00:00`) : new Date(`${labelRange.end_date}T12:00:00`),
