...
export function buildInitialScheduleForm({ subject, planData, academicYearId }) {
  ...
  const startDateStr = String(plan?.start_date || planData?.start_date || '').slice(0, 10);
  const endDateStr = String(plan?.end_date || planData?.end_date || '').slice(0, 10);

  return {
    weekdays: weekdays.length > 0 ? weekdays : [],
    startTime: blockStartTime,
    durationMinutes: durationMinutes != null && durationMinutes > 0 ? durationMinutes : '',
    startDate: DATE_KEY_RE.test(startDateStr) ? new Date(`${startDateStr}T12:00:00`) : null,
    endDate: DATE_KEY_RE.test(endDateStr) ? new Date(`${endDateStr}T12:00:00`) : null,
