...
export function getSubjectTermDateRange(schoolYearLabel, schoolTermId, plannerSettings = null) {
  ...
  const summerStart = String(plannerSettings?.default_summer_term_start_date || '').slice(0, 10)
    || (Number.isFinite(endYear) ? `${endYear}-06-01` : range.start_date);
  const summerEnd = String(plannerSettings?.default_summer_term_end_date || '').slice(0, 10)
    || (Number.isFinite(endYear) ? `${endYear}-08-31` : range.end_date);
  ...
  if (term === 'summer_term') return { start_date: summerStart, end_date: summerEnd };
  // Full year: guarantee a valid range even if the saved year dates are missing/invalid
  // by falling back to the fall-term start and spring-term end.
  const fullStart = DATE_KEY_RE.test(yearStart) ? yearStart : fallStart;
  const fullEnd = DATE_KEY_RE.test(yearEnd) ? yearEnd : springEnd;
  return { start_date: fullStart, end_date: fullEnd };
}
...
async function clearSubjectPlanPlaceholders({ familyId, subjectId, academicYearId = null }) {
  ...
  // ClassDay slots often have subject_id null and only link via curriculum_metadata.subject_ids.
  try {
    let metaQuery = supabase
      .from('events')
      .select('id, curriculum_metadata, title')
      .eq('family_id', familyId)
      .is('deleted_at', null)
      .contains('curriculum_metadata', { subject_ids: [sid] });
    if (academicYearId) {
      metaQuery = metaQuery.eq('academic_year_id', String(academicYearId));
    }
    const { data: metaRows } = await metaQuery;
    const toDelete = [];
    for (const row of metaRows || []) {
      const raw = row?.curriculum_metadata;
      let parsed = raw;
      if (typeof parsed === 'string') {
        try { parsed = JSON.parse(parsed); } catch (_) { parsed = {}; }
      }
      const ids = Array.isArray(parsed?.subject_ids)
        ? parsed.subject_ids.map((id) => String(id || '').trim()).filter(Boolean)
        : [];
      ...
      if (remaining.length === 0) {
        ...
        await supabase
          .from('events')
          .update({
            curriculum_metadata: { ...(parsed || {}), subject_ids: remaining },
            updated_at: now,
          })
          .eq('id', row.id)
          .is('deleted_at', null);
    ...
    console.warn('[subjectConfigureSchedule] ClassDay metadata cleanup:', err?.message || err);
  }
}
...
export async function removeSubjectScheduleFromCalendar({
  ...
  // Metadata-linked ClassDays (subject_id null) — clearSubjectPlanPlaceholders handles those.
  await clearSubjectPlanPlaceholders({ familyId, subjectId: sid, academicYearId });
...
/** Find a family academic year that overlaps the schedule range (for brand-new subjects). */
export async function resolveFamilyAcademicYearForRange(familyId, startYmd, endYmd) {
  if (!familyId || !DATE_KEY_RE.test(startYmd) || !DATE_KEY_RE.test(endYmd)) {
    return { academicYearId: null, planData: null };
  }
  const { data: rows, error } = await supabase
    .from('academic_years')
    .select('id, start_date, end_date, updated_at, year_name')
    .eq('family_id', familyId)
  ...
  const overlapping = rows.find((row) => {
    const rowStart = String(row?.start_date || '').slice(0, 10);
    const rowEnd = String(row?.end_date || '').slice(0, 10);
    if (!DATE_KEY_RE.test(rowStart) || !DATE_KEY_RE.test(rowEnd)) return false;
    return rowStart <= endYmd && rowEnd >= startYmd;
  });
  const chosen = overlapping || rows[0];
  if (!chosen?.id) return { academicYearId: null, planData: null };

  const { data, error: yearError } = await getAcademicYear(chosen.id);
  if (yearError) return { academicYearId: String(chosen.id), planData: null };
  return { academicYearId: String(chosen.id), planData: data || null };
}
...
export async function applySubjectScheduleToCalendar({
  ...
  // Brand-new subjects are not in any plan yet — attach to an overlapping family year
  // instead of bootstrapping a one-subject orphan year.
  if (!yearId) {
    const familyYear = await resolveFamilyAcademicYearForRange(familyId, startYmd, endYmd);
    yearId = familyYear.academicYearId || null;
    yearDetail = familyYear.planData || yearDetail;
  }
