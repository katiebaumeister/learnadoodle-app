...
export function buildSubjectClassworkModel({ units = [], assignments = [], events = [] }) {
  ...
  const unitSections = (units || []).map((unit, index) => {
    ...
    const lessons = (unit?.lessons || []).map((lesson, li) => {
      ...
      if (lessonId && isPersistedCurriculumId(lessonId)) {
        lessonToUnitId.set(lessonId, unitId);
      }
      const schedule = lessonId && isPersistedCurriculumId(lessonId)
        ? (lessonScheduleById.get(lessonId) || null)
        : null;
      const learningDays = lessonId && isPersistedCurriculumId(lessonId)
