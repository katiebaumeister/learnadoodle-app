...
import { isDayOffOrHolidayEvent } from '../create/eventOpenRouting';
import { dispatchOpenSchoolYearSettingsModal } from '../planYearRetirement';

export const OPEN_LEARNING_DAY_MODAL_EVENT = 'openLearningDayModal';

export function dispatchOpenLearningDayModal({
  event = null,
  eventId = null,
  skipSummary = false,
} = {}) {
...
export function dispatchOpenLearningDayChoiceModal({ event = null, eventId = null } = {}) {
  dispatchOpenLearningDayModal({ event, eventId });
}
...
export function learningDayEventSelectFields() {
  return 'id, subject_id, title, description, material_id, materials_attachment_ids, generated_by, source_block_id, academic_year_id, curriculum_lesson_id, curriculum_metadata, event_type, start_ts, end_ts, child_id, status, canceled_at, minutes, lesson, unit';
}
...
/** Resolve calendar event id from a subject overview schedule row. */
export function resolveScheduleWorkItemEventId(item) {
  if (!item) return null;
  if (item.eventId) return String(item.eventId).trim().replace(/^event-/, '');
  if (item.event_id) return String(item.event_id).trim().replace(/^event-/, '');
  const rawId = item.id != null ? String(item.id).trim() : '';
  if (!rawId) return null;
  const prefixed = rawId.match(/^(?:event|assignment|backlog)-(.+)$/i);
  if (prefixed?.[1]) return prefixed[1];
  return rawId.replace(/^event-/, '');
}

export function buildScheduleWorkItemEventStub(item, subjectId = null) {
  const eventId = resolveScheduleWorkItemEventId(item);
  ...
  return {
    ...
    start_ts: item?.startTs || item?.start_ts || item?.dueDate || item?.due_ts || null,
    end_ts: item?.endTs || item?.end_ts || null,
    child_id: item?.childId || item?.child_id || null,
    subject_id: item?.subjectId || item?.subject_id || subjectId || null,
    event_type: eventType,
    status: item?.status || null,
  };
}

/** Open the correct modal for a subject card “coming up” row (event, assignment, or learning day). */
export function dispatchOpenScheduleWorkItemModal(item, { subjectId = null } = {}) {
  if (Platform.OS !== 'web' || typeof window === 'undefined' || !item) return false;
  const event = buildScheduleWorkItemEventStub(item, subjectId);
  ...
  if (getPlannerEventCategory(event) === 'Learning day') {
