...
function localYmdFromValue(value) {
  if (!value) return '';
  // Pure calendar dates are already local YYYY-MM-DD — keep as-is.
  if (typeof value === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(value.trim())) {
    return value.trim().slice(0, 10);
  }
  // ISO timestamptz (e.g. 2026-08-23T00:00:00.000Z) must use local timezone —
  // slicing the UTC date prefix shifts untimed/evening events onto the wrong day
  // and makes Home disagree with Planner's get_month_view buckets.
...
function flattenEventsByDate(eventsByDate = {}) {
  ...
  Object.entries(eventsByDate || {}).forEach(([dateKey, day]) => {
    ...
    const bucketYmd = /^\d{4}-\d{2}-\d{2}$/.test(String(dateKey || '').slice(0, 10))
      ? String(dateKey).slice(0, 10)
      : '';
    list.forEach((event) => {
      if (!event) return;
      // Trust Planner's day bucket over start_ts UTC prefix.
      all.push(bucketYmd
        ? { ...event, date_local: event.date_local || bucketYmd }
        : event);
    });
  });
  return all;
...
export function scheduleEventsForYmdFromPlannerMap(eventsByDate, ymd) {
  if (!ymd) return [];
  // Start from the Planner day bucket (timezone-correct from get_month_view).
  const dayBucket = eventsByDate?.[ymd];
  const dayList = Array.isArray(dayBucket) ? dayBucket : (dayBucket?.events || []);
  const fromBucket = (dayList || []).map((event) => (
    event ? { ...event, date_local: event.date_local || ymd } : event
  ));

  // Also include multi-day Project/Break rows that span into this day but may
  // only be keyed under their start date.
  const all = flattenEventsByDate(eventsByDate);
  const expanded = expandRangeEventsLikePlanner(all);
  const spanning = expanded.filter((event) => {
    if (!eventOccursOnYmd(event, ymd)) return false;
    const id = String(event._originalId || event.id || '');
    return fromBucket.every((row) => String(row?._originalId || row?.id || '') !== id);
  });

  const forDay = [...fromBucket, ...spanning];
