 ...
 * Chips use neutral light-grey fills (public holidays stay blank).
 */

const PLANNER_CHIP_TEXT = '#374151';
const PLANNER_CHIP_FILL = '#F8FAFC';
const PLANNER_CHIP_FILL_HOVER = '#F1F5F9';

const FILLED_CHIP = {
  color: PLANNER_CHIP_FILL,
  borderColor: 'transparent',
  chipText: PLANNER_CHIP_TEXT,
  accent: PLANNER_CHIP_TEXT,
  hoverColor: PLANNER_CHIP_FILL_HOVER,
  hoverBorderColor: 'transparent',
};

export const PLANNER_EVENT_CATEGORIES = [
  { key: 'Event', label: 'Event', ...FILLED_CHIP },
  { key: 'Assignment', label: 'Assignment', ...FILLED_CHIP },
  { key: 'Learning day', label: 'Learning day', ...FILLED_CHIP },
  { key: 'Day off', label: 'Day off', ...FILLED_CHIP },
];
...
export function getPlannerEventTypeColors(event) {
  ...
  return {
    ...
    chipBorder: meta.borderColor,
    hoverBg: meta.hoverColor,
    hoverBorder: meta.hoverBorderColor,
...
export function getPlannerCalendarLegendItems() {
  return PLANNER_EVENT_CATEGORIES.map(({ label, color }) => ({
    label,
    color: color || PLANNER_CHIP_FILL,
  }));
}
