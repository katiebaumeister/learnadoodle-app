...
import { studentResponseTypeLabel, parseStudentResponseType, STUDENT_RESPONSE_NO_RESPONSE } from '../studentResponseTypes';
...
export function buildPlannerItemSummaryModel({
  ...
  if (resolvedCategory === 'Assignment') {
    ...
    const parsedResponseType = parseStudentResponseType(workSpec?.student_response_type);
    const responseType = parsedResponseType && parsedResponseType !== STUDENT_RESPONSE_NO_RESPONSE
      ? studentResponseTypeLabel(parsedResponseType)
      : '';
    const unitTitle = resolveUnitTitle(event);
    const lessonTitle = resolveLessonTitle(event);
    const attachCount = attachmentCount(event);
