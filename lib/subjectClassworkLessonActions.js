...
function defaultUnitDraft(unitIndex = 0, { includeLesson = true } = {}) {
...
export function buildDraftWithAddedUnit(units) {
  const existing = draftFromCurriculumStructure({ units });
  const nextUnitIndex = existing?.units?.length || 0;
  const nextUnit = defaultUnitDraft(nextUnitIndex, { includeLesson: true });
  if (!existing) {
