...
export class AIConversationService {
  ...
  static async getLatestDoodleConversation(familyId) {
    try {
      const list = await this.getConversationHistory(familyId, 'doodlebot', 1);
      ...
      const messages = rawMessages
        .map((m) => ({
          ...
          timestamp: m.timestamp ? new Date(m.timestamp).getTime() : (m.created_at ? new Date(m.created_at).getTime() : Date.now()),
        }))
