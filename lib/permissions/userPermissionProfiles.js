const CHILD_PROFILE_VALUES = ['guided', 'standard', 'independent', 'self_managed'];
...
const CHILD_PROFILE_PERMISSIONS = {
  ...
  standard: {
    ...
    // Children are allowed to add/edit their own events (planner) by default.
  ...
  self_managed: {
    ...
    canMarkComplete: true,
    canLogProgress: true,
    // Self-managed student accounts should retain full settings control
    // until they are linked into a parent-managed family.
    canManagePlans: true,
    canChangePlanningPreferences: true,
...
/**
 * Whether the signed-in viewer may toggle event "done" (completion ring / checklist).
 * Independent of create/edit event rights — children should mark their own work done
 * even when parents haven't enabled add/edit events.
 */
export function canViewerMarkEventsComplete(session, effectivePermissions = null) {
  const roleFlags = session?.role_flags || {};
  if (roleFlags.isTutor === true) return false;
  if (roleFlags.isChild === true) {
    if (effectivePermissions && Object.prototype.hasOwnProperty.call(effectivePermissions, 'canMarkComplete')) {
      return !!effectivePermissions.canMarkComplete;
    }
    return true;
  }
  if (roleFlags.isParent === true || session?.effective_role === 'parent') return true;
  return true;
}
