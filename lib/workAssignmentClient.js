...
import { dispatchAssignmentRefreshEvents, getChildIdsFromEvent } from './assignmentWorkflowClient';
import { logActivityFromAssignment } from './services/assignmentActivityClient';
...
async function findAssignmentForChild({ familyId, childId, eventId }) {
  const { data: rows, error } = await supabase
    .from('assignments')
    .select('id, title, linked_event_ids, child_id, status, review_status, progress_percent, grade_display, grade_value, submitted_at, start_work_by')
...
export async function ensureAssignmentsForEvent({
  ...
  const parsedSpec = parseWorkSpec(workSpec || event.work_spec, event.event_type);
  const startWorkBy = (() => {
    if (String(parsedSpec.suggested_start_mode || '').toLowerCase() === 'custom' && parsedSpec.suggested_start_date) {
      const ymd = String(parsedSpec.suggested_start_date).slice(0, 10);
      if (/^\d{4}-\d{2}-\d{2}$/.test(ymd)) return ymd;
    }
    return computeSuggestedStartDate(event, parsedSpec);
  })();
  ...
  for (const childId of targetChildIds) {
    ...
    const { data: createdRow, error: insErr } = await createAssignment({
      family_id: familyId,
      child_id: childId,
      title,
    ...
    if (createdRow?.id) {
      await logActivityFromAssignment(createdRow, ACTIVITY_TYPE.ASSIGNED);
    }
  }
...
export async function fetchUpcomingWorkForChild({
  ...
  const { data: assignments, error: assignError } = await supabase
    .from('assignments')
    .select('id, child_id, status, review_status, submitted_at, progress_percent, grade_display, grade_value, linked_event_ids, need_help, due_date, start_work_by, title')
    .eq('family_id', familyId)
...
export async function reviewAssignmentWork({
  ...
  if (Array.isArray(reviewAttachmentIds)) {
    patch.linked_review_attachment_ids = reviewAttachmentIds.map(String).filter(Boolean);
  }

  if (action === 'save') {
    patch.grade_display = gradeDisplay != null ? String(gradeDisplay) : null;
    if (gradeValue != null && Number.isFinite(Number(gradeValue))) {
      patch.grade_value = Number(gradeValue);
    } else {
      patch.grade_value = null;
    }
  } else {
    patch.reviewed_at = new Date().toISOString();
    patch.reviewed_by = reviewerId || null;
  ...
  const { data: row } = await supabase
    .from('assignments')
    .select('id, family_id, child_id, title, related_subject, status, review_status')
    .eq('id', assignmentId)
    .maybeSingle();

  if (row) {
    if (action === 'send_back') {
      await logActivityFromAssignment(row, ACTIVITY_TYPE.RETURNED);
    } else if (action === 'approve' || action === 'grade') {
      await logActivityFromAssignment(row, ACTIVITY_TYPE.COMPLETED);
    }
  }
