...
export class PlannerService {
   ...
   * @param {string} childId - Child ID (optional if session provided)
   * @param {string} fromDate - Start date (YYYY-MM-DD)
   * @param {string} toDate - End date (YYYY-MM-DD)
   * @param {Object} [session] - Session context for role-based filtering
   */
  async getExistingEvents(childId, fromDate, toDate, session = null) {
    let query = supabase
      .from('events')
      .select('*')
      .eq('status', 'scheduled')
