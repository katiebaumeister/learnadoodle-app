...
import { HOME_GETTING_STARTED_SYSTEM_KIND } from './homeWelcomeBulletin';
...
export function postToStreamCard(post, { subjectName = null, authorName = 'Parent' } = {}) {
  ...
  if (hasFormattedBullets) {
    ...
    return {
      ...
      label: (post.systemKind === 'subject_getting_started' || isHomeWelcome) ? 'Welcome' : 'Announcement',
...
export function buildStreamPreviewDisplay(entry, { showSubjectName = false } = {}) {
  ...
  if (entry?.kind === 'post' && entry.showFormattedBody) {
    const welcomeTitle = entry.payload?.systemKind === SUBJECT_GETTING_STARTED_SYSTEM_KIND && entry.subjectName
      ? `Welcome to ${entry.subjectName}`
      : null;
    return {
      label: entry.label,
      title: welcomeTitle || entry.subjectName || entry.title,
