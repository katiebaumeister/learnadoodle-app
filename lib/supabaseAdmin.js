...
export const testAdminAccess = async () => {
  try {
    ...
    // Test academic_years
    const { data: academicYears, error: academicError } = await supabaseAdmin
      .from('academic_years')
      .select('*')
      .limit(1);
    
    if (academicError) {
      console.error('❌ Admin academic_years error:', academicError);
    } else {
      console.log('✅ Admin academic_years access:', academicYears);
    }
    ...
    return { academicYears, holidays, academicError, holidayError };
  } catch (error) {
