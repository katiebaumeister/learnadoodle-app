...
/** Send a nudge (reminder) for one event to one or more assigned students. */
export async function sendNudgeForEvent({
  familyId,
  event,
  childIds = null,
  userId = null,
  ...
  if (targetChildIds.length === 0) {
    throw new Error('No student assigned to this event');
  }
  ...
  const message = String(note || '').trim() || `Reminder: ${title}`;
  ...
  for (const childId of targetChildIds) {
    ...
    if (!assignmentId) {
      throw new Error('Could not link assignment');
    }
    await appendAssignmentMessage(assignmentId, message, 'sent_assignment');
  }

  dispatchAssignmentRefreshEvents();
  return { childCount: targetChildIds.length };
}
