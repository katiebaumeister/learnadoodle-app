...
export async function applyLearningDayTimeOverride({
  ...
  let sessionDate = getEventStartDate(event);
  if (sessionDateYmd) {
    const parts = String(sessionDateYmd).slice(0, 10).split('-').map((part) => parseInt(part, 10));
    if (parts.length === 3 && parts.every((n) => Number.isFinite(n))) {
      sessionDate = new Date(parts[0], parts[1] - 1, parts[2]);
    }
  }
  if (!sessionDate || Number.isNaN(sessionDate.getTime())) throw new Error('Missing session date');
