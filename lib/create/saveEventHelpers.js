/**
 * Shared create/save helpers for Calendar Event, Lesson, and Assignment modals.
 * Core objects: Subject → Units → Lessons, Material, Assignment, Event.
 */
import { Platform } from 'react-native';
import { supabase } from '../supabase';
import { ensureAssignmentsForEvent } from '../workAssignmentClient';
import { defaultWorkSpec, parseWorkSpec } from '../workEventHelpers';
import { computeEventTimes, toYmd, validateOptionalEventTimes } from './eventTimeUtils';
import { updateEvent } from '../services/plannerClientWithOffline';

const DAY_OFF_EXCLUSION_TYPES = ['holiday', 'break', 'day_off', 'dayoff', 'day-off', 'no_school', 'custom_holiday', 'custom_break'];

async function resolveFamilyId(familyId) {
  if (familyId) return familyId;
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('User not authenticated');
  const { data: profile, error } = await supabase
    .from('profiles')
    .select('family_id')
    .eq('id', user.id)
    .single();
  if (error || !profile?.family_id) throw new Error('Failed to fetch family information');
  return profile.family_id;
}

function childPayload(childIds) {
  const ids = (Array.isArray(childIds) ? childIds : []).filter(Boolean);
  return {
    childId: ids[0] || null,
    childIds: ids.length > 0 ? ids : null,
  };
}

async function findDayOffForDate(familyId, date) {
  const dateYmd = toYmd(date);
  if (!familyId || !dateYmd) return null;

  const { data, error } = await supabase
    .from('planner_exclusions')
    .select('id, label, exclusion_type, start_date, end_date')
    .eq('family_id', familyId)
    .eq('is_active', true)
    .in('exclusion_type', DAY_OFF_EXCLUSION_TYPES)
    .lte('start_date', dateYmd)
    .gte('end_date', dateYmd)
    .limit(1);

  if (error) {
    console.warn('[saveEventHelpers] Could not check saved days off:', error.message || error);
    return null;
  }

  return Array.isArray(data) && data.length > 0 ? data[0] : null;
}

async function assertDateIsNotDayOff(familyId, date, { isRepeating = false } = {}) {
  const dayOff = await findDayOffForDate(familyId, date);
  if (!dayOff) return;
  const label = String(dayOff.label || '').trim();
  const prefix = label ? `${label} is marked as a day off.` : 'That date is marked as a day off.';
  const action = isRepeating
    ? 'Choose a different first date for this repeating event, or remove the day off first.'
    : 'Choose a different date, or remove the day off first.';
  throw new Error(`${prefix} ${action}`);
}

async function invokeCreateTaskEvent(params) {
  const { data: rpcData, error: rpcError } = await supabase.rpc('create_task_event', params);
  if (rpcError || !rpcData?.ok) {
    throw new Error(rpcError?.message || rpcData?.error || 'Failed to create event');
  }
  const { data: eventData, error: fetchError } = await supabase
    .from('events')
    .select('*')
    .eq('id', rpcData.id)
    .single();
  if (fetchError) throw fetchError;
  return eventData;
}

function dispatchRefresh(detail = null) {
  if (Platform.OS === 'web' && typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('refreshCalendar', detail ? { detail } : undefined)
    );
  }
}

// Recurring saves can add/move/remove instances across many months and weeks.
// A plain refresh only refetches the visible month, leaving prefetched neighbour
// months (and the week-view cache) stale until the user navigates away and back.
// Force a full invalidate so every cached range refetches immediately.
const RECURRING_REFRESH_DETAIL = { forceInvalidate: true };

function dispatchSubjectRefresh(subjectId) {
  if (!subjectId || Platform.OS !== 'web' || typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent('refreshSubjectDetail', { detail: { subjectId } }));
}

export async function saveCalendarEvent({
  familyId,
  title,
  childIds,
  date,
  startTime = '',
  endTime = '',
  allDay = false,
  endDate = null,
  location = '',
  notes = '',
  materialIds = [],
  subjectId = null,
  countsTowardPlan = false,
  recurrenceRule = null,
  isBacklog = false,
  allowConflict = false,
}) {
  const userFamilyId = await resolveFamilyId(familyId);
  const { childId, childIds: payloadChildIds } = childPayload(childIds);
  if (!isBacklog) {
    await assertDateIsNotDayOff(userFamilyId, date, { isRepeating: !!recurrenceRule });
  }

  let startTs;
  let endTs;
  let minutes = null;
  let isFlexible = false;

  if (isBacklog) {
    const base = date instanceof Date ? date : new Date();
    base.setHours(12, 0, 0, 0);
    startTs = base.toISOString();
    endTs = new Date(base.getTime() + 30 * 60 * 1000).toISOString();
  } else {
    const timeCheck = validateOptionalEventTimes({ startTime, endTime });
    if (!timeCheck.ok) {
      throw new Error(timeCheck.error);
    }
    const times = computeEventTimes({ date, endDate, startTime, endTime, allDay, allowOptionalTime: true });
    if (!times.start || !times.end) {
      throw new Error('Date is required');
    }
    startTs = times.start.toISOString();
    endTs = times.end.toISOString();
    minutes = times.minutes;
    // allowConflict marks the event flexible so the DB overlap guard permits it.
    isFlexible = (!allDay && !!times.isFlexible) || !!allowConflict;
  }

  const event = await invokeCreateTaskEvent({
    _family_id: userFamilyId,
    _child_id: childId,
    _child_ids: payloadChildIds,
    _title: String(title || '').trim(),
    _start_ts: startTs,
    _end_ts: endTs,
    _description: notes?.trim() || null,
    _status: 'scheduled',
    _source: 'manual',
    _tags: null,
    _is_flexible: isFlexible,
    _is_backlog: !!isBacklog,
    _event_type: 'Other',
    _subject_id: subjectId || null,
    _unit: null,
    _grade: null,
    _percent_of_total_grade: null,
    _location: location?.trim() || null,
    _mode: null,
    _instructor: null,
    _goal_link: null,
    _minutes: minutes,
    _materials_attachment_ids: materialIds?.length ? materialIds : null,
    _recurrence_rule: recurrenceRule ? JSON.stringify(recurrenceRule) : null,
  });

  const patch = {
    counts_toward_plan: countsTowardPlan !== false,
    subject_id: subjectId || null,
    // Explicit all-day vs optional blank time (is_flexible). Never leave all_day inferred.
    all_day: !!allDay && !isFlexible,
    is_flexible: !!isFlexible,
  };

  const { data: updated, error: patchError } = await supabase
    .from('events')
    .update(patch)
    .eq('id', event.id)
    .select('*')
    .single();
  if (patchError) throw patchError;

  if (!isBacklog) dispatchRefresh(recurrenceRule ? RECURRING_REFRESH_DETAIL : null);
  return updated || { ...event, ...patch };
}

export async function updateCalendarEvent({
  eventId,
  familyId,
  title,
  childIds,
  date,
  startTime = '',
  endTime = '',
  endDate = null,
  location = '',
  notes = '',
  materialIds = [],
  recurrenceRule = null,
  curriculumLessonId = undefined,
  unitTitle = '',
  lessonLabel = '',
  allowConflict = false,
}) {
  if (!eventId) throw new Error('Missing event');

  const userFamilyId = await resolveFamilyId(familyId);
  const { childId, childIds: payloadChildIds } = childPayload(childIds);
  const timeCheck = validateOptionalEventTimes({ startTime, endTime });
  if (!timeCheck.ok) {
    throw new Error(timeCheck.error);
  }
  const times = computeEventTimes({ date, endDate, startTime, endTime, allowOptionalTime: true });
  if (!times.start || !times.end) {
    throw new Error('Date is required');
  }

  const updates = {
    title: String(title || '').trim(),
    start_ts: times.start.toISOString(),
    end_ts: times.end.toISOString(),
    description: notes?.trim() || null,
    location: location?.trim() || null,
    child_id: childId,
    child_ids: payloadChildIds,
    minutes: times.minutes,
    is_flexible: !!times.isFlexible || !!allowConflict,
    all_day: false,
    materials_attachment_ids: materialIds?.length ? materialIds : null,
    material_id: materialIds?.[0] || null,
    recurrence_rule: recurrenceRule ? JSON.stringify(recurrenceRule) : null,
  };

  if (curriculumLessonId !== undefined) {
    if (curriculumLessonId) {
      const unit = String(unitTitle || '').trim() || null;
      const lesson = String(lessonLabel || '').trim() || null;
      updates.curriculum_lesson_id = String(curriculumLessonId);
      updates.curriculum_unit_title = unit;
      updates.unit = unit;
      updates.lesson = lesson;
      updates.curriculum_metadata = lesson ? { lesson_label: lesson } : {};
    } else {
      updates.curriculum_lesson_id = null;
      updates.curriculum_unit_title = null;
      updates.unit = null;
      updates.lesson = null;
      updates.curriculum_metadata = {};
    }
  }

  const { data, error } = await updateEvent(eventId, updates, userFamilyId);
  if (error) throw error;

  dispatchRefresh(recurrenceRule ? RECURRING_REFRESH_DETAIL : null);
  return data;
}

/**
 * Apply an edit to EVERY occurrence in a recurring series (the "Edit series" flow).
 *
 * Shared fields (title, assignees, location, notes, materials, recurrence rule) are
 * written to all linked rows, and the new time-of-day is applied to each occurrence
 * while preserving that occurrence's own date. This handles the common case ("move the
 * whole series an hour later"). Note: changing the set of weekdays / end here updates the
 * stored rule but does not add or remove existing occurrences.
 */
export async function updateCalendarEventSeries({
  eventId,
  familyId,
  title,
  childIds,
  startTime = '',
  endTime = '',
  location = '',
  notes = '',
  materialIds = [],
  recurrenceRule = null,
  curriculumLessonId = undefined,
  unitTitle = '',
  lessonLabel = '',
  allowConflict = false,
}) {
  if (!eventId) throw new Error('Missing event');

  const userFamilyId = await resolveFamilyId(familyId);
  const { childId, childIds: payloadChildIds } = childPayload(childIds);
  const timeCheck = validateOptionalEventTimes({ startTime, endTime });
  if (!timeCheck.ok) {
    throw new Error(timeCheck.error);
  }

  const { data: anchor, error: anchorError } = await supabase
    .from('events')
    .select('id, recurrence_id, parent_event_id')
    .eq('id', eventId)
    .maybeSingle();
  if (anchorError) throw anchorError;

  const linkIds = Array.from(new Set([
    eventId,
    anchor?.recurrence_id,
    anchor?.parent_event_id,
  ].filter(Boolean).map(String)));

  const filterClauses = linkIds.flatMap((id) => [
    `id.eq.${id}`,
    `recurrence_id.eq.${id}`,
    `parent_event_id.eq.${id}`,
  ]);

  const { data: rows, error: rowsError } = await supabase
    .from('events')
    .select('id, start_ts')
    .eq('family_id', userFamilyId)
    .is('deleted_at', null)
    .or(filterClauses.join(','));
  if (rowsError) throw rowsError;

  const seriesRows = Array.isArray(rows) && rows.length > 0
    ? rows
    : [{ id: eventId, start_ts: null }];

  const sharedUpdates = {
    title: String(title || '').trim(),
    description: notes?.trim() || null,
    location: location?.trim() || null,
    child_id: childId,
    child_ids: payloadChildIds,
    all_day: false,
    materials_attachment_ids: materialIds?.length ? materialIds : null,
    material_id: materialIds?.[0] || null,
    recurrence_rule: recurrenceRule ? JSON.stringify(recurrenceRule) : null,
  };

  if (curriculumLessonId !== undefined) {
    if (curriculumLessonId) {
      const unit = String(unitTitle || '').trim() || null;
      const lesson = String(lessonLabel || '').trim() || null;
      sharedUpdates.curriculum_lesson_id = String(curriculumLessonId);
      sharedUpdates.curriculum_unit_title = unit;
      sharedUpdates.unit = unit;
      sharedUpdates.lesson = lesson;
      sharedUpdates.curriculum_metadata = lesson ? { lesson_label: lesson } : {};
    } else {
      sharedUpdates.curriculum_lesson_id = null;
      sharedUpdates.curriculum_unit_title = null;
      sharedUpdates.unit = null;
      sharedUpdates.lesson = null;
      sharedUpdates.curriculum_metadata = {};
    }
  }

  let lastError = null;
  let updatedCount = 0;
  for (const row of seriesRows) {
    const rowDate = row.start_ts ? new Date(row.start_ts) : new Date();
    if (Number.isNaN(rowDate.getTime())) continue;
    const times = computeEventTimes({ date: rowDate, startTime, endTime, allowOptionalTime: true });
    if (!times.start || !times.end) continue;
    const updates = {
      ...sharedUpdates,
      start_ts: times.start.toISOString(),
      end_ts: times.end.toISOString(),
      minutes: times.minutes,
      is_flexible: !!times.isFlexible || !!allowConflict,
    };
    const { error } = await updateEvent(row.id, updates, userFamilyId);
    if (error) lastError = error;
    else updatedCount += 1;
  }

  if (updatedCount === 0 && lastError) throw lastError;

  // A series edit always touches multiple rows across the calendar.
  dispatchRefresh(RECURRING_REFRESH_DETAIL);
  return { id: eventId, updatedCount };
}

export async function saveLesson({
  familyId,
  title,
  childIds,
  subjectId,
  unitTitle = '',
  curriculumLessonId = null,
  lessonLabel = '',
  description = '',
  materialIds = [],
  durationMinutes = 60,
  objectives = '',
  delivery = '',
  scheduleMode = 'schedule_now',
  date = new Date(),
  startTime = '',
  endTime = '',
  sendToStudent = true,
}) {
  const userFamilyId = await resolveFamilyId(familyId);
  const { childId, childIds: payloadChildIds } = childPayload(childIds);
  const isBacklog = scheduleMode === 'backlog';
  const isUnscheduled = scheduleMode === 'unscheduled';

  let startTs;
  let endTs;
  let minutes = Number(durationMinutes) || 60;

  if (isUnscheduled || isBacklog) {
    const base = date instanceof Date ? date : new Date();
    base.setHours(12, 0, 0, 0);
    startTs = base.toISOString();
    endTs = new Date(base.getTime() + minutes * 60 * 1000).toISOString();
  } else {
    const times = computeEventTimes({ date, startTime, endTime, durationMinutes: minutes });
    if (!times.start || !times.end) throw new Error('Choose a date and valid times to schedule this lesson');
    startTs = times.start.toISOString();
    endTs = times.end.toISOString();
    minutes = times.minutes || minutes;
  }

  // Empty learning days (no curriculum lesson / no explicit lesson label) must not
  // inherit the event title as lesson — Classwork treats that as a curriculum link.
  const resolvedLessonLabel = lessonLabel?.trim()
    || (curriculumLessonId ? title?.trim() : '')
    || null;

  const curriculumMetadata = {
    objectives: objectives?.trim() || null,
    delivery: delivery?.trim() || null,
    send_to_student: sendToStudent !== false,
    lesson_label: resolvedLessonLabel,
  };

  const event = await invokeCreateTaskEvent({
    _family_id: userFamilyId,
    _child_id: childId,
    _child_ids: payloadChildIds,
    _title: String(title || '').trim(),
    _start_ts: startTs,
    _end_ts: endTs,
    _description: description?.trim() || null,
    _status: 'scheduled',
    _source: 'manual',
    _tags: null,
    _is_flexible: isUnscheduled,
    _is_backlog: isBacklog,
    _event_type: 'Lesson',
    _subject_id: subjectId || null,
    _unit: unitTitle?.trim() || null,
    _grade: null,
    _percent_of_total_grade: null,
    _location: null,
    _mode: delivery?.trim() || null,
    _instructor: null,
    _goal_link: null,
    _minutes: minutes,
    _materials_attachment_ids: materialIds?.length ? materialIds : null,
    _recurrence_rule: null,
  });

  const patch = {
    curriculum_metadata: curriculumMetadata,
    curriculum_unit_title: unitTitle?.trim() || null,
    lesson: resolvedLessonLabel,
    curriculum_lesson_id: curriculumLessonId || null,
    subject_id: subjectId || null,
    counts_toward_plan: true,
  };

  const { data: updated, error: patchError } = await supabase
    .from('events')
    .update(patch)
    .eq('id', event.id)
    .select('*')
    .single();
  if (patchError) throw patchError;

  if (scheduleMode === 'schedule_now' || isUnscheduled || isBacklog) {
    dispatchRefresh(isUnscheduled || isBacklog ? { forceInvalidate: true } : null);
    dispatchSubjectRefresh(subjectId);
  }
  return updated || { ...event, ...patch };
}

const ASSIGNMENT_TYPE_TO_EVENT = {
  Project: 'Project',
  Exam: 'Exam',
};

export async function saveAssignment({
  familyId,
  title,
  childIds,
  subjectId,
  instructions = '',
  assignmentType = 'Assignment',
  workSpecInput = null,
  availableDate = null,
  dueDate = null,
  gradingMode = 'ungraded',
  materialIds = [],
  allowResubmission = false,
  requireParentApproval = false,
  unitTitle = '',
  unitId = null,
  curriculumLessonId = null,
  lessonLabel = '',
  rubricId = null,
  points = null,
  milestoneDueDate = null,
  saveMode = 'assign',
  releaseDate = null,
  linkedLearningDayEventId = null,
}) {
  const userFamilyId = await resolveFamilyId(familyId);
  const { childId, childIds: payloadChildIds } = childPayload(childIds);
  const mode = String(saveMode || 'assign').trim().toLowerCase();
  const isDraft = mode === 'draft';
  const isScheduled = mode === 'schedule';
  const effectiveReleaseDate = isScheduled ? (releaseDate || availableDate) : availableDate;

  const eventType = ASSIGNMENT_TYPE_TO_EVENT[assignmentType] || 'Assignment';
  const baseSpec = defaultWorkSpec(eventType);
  const workSpec = parseWorkSpec(
    {
      ...baseSpec,
      ...(workSpecInput || {}),
      instructions: instructions?.trim() || '',
      graded: gradingMode !== 'ungraded',
      assignment_type: assignmentType,
      allow_resubmission: !!allowResubmission,
      require_parent_approval: !!requireParentApproval,
      grading_mode: gradingMode,
      suggested_start_mode: effectiveReleaseDate ? 'custom' : 'auto',
      suggested_start_date: effectiveReleaseDate ? toYmd(effectiveReleaseDate) : null,
      rubric_id: rubricId || workSpecInput?.rubric_id || null,
      points_possible: points || workSpecInput?.points_possible || null,
      milestone_due_date: milestoneDueDate ? toYmd(milestoneDueDate) : null,
    },
    eventType,
  );

  let startTs;
  let endTs;
  if (isDraft) {
    const placeholder = new Date();
    placeholder.setHours(12, 0, 0, 0);
    startTs = placeholder.toISOString();
    endTs = new Date(placeholder.getTime() + 30 * 60 * 1000).toISOString();
  } else {
    const scheduleDate = dueDate || effectiveReleaseDate || new Date();
    const times = computeEventTimes({ date: scheduleDate, startTime: '', endTime: '', allDay: true });
    startTs = times.start?.toISOString() || new Date().toISOString();
    endTs = times.end?.toISOString() || startTs;
  }

  const event = await invokeCreateTaskEvent({
    _family_id: userFamilyId,
    _child_id: childId,
    _child_ids: payloadChildIds,
    _title: String(title || '').trim(),
    _start_ts: startTs,
    _end_ts: endTs,
    _description: instructions?.trim() || null,
    _status: 'scheduled',
    _source: 'manual',
    _tags: null,
    _is_flexible: true,
    _is_backlog: isDraft,
    _event_type: eventType,
    _subject_id: subjectId || null,
    _unit: unitTitle?.trim() || null,
    _grade: gradingMode === 'points' && points ? String(points) : null,
    _percent_of_total_grade: gradingMode === 'percentage' ? 100 : null,
    _location: null,
    _mode: null,
    _instructor: null,
    _goal_link: null,
    _minutes: workSpec.estimated_effort_minutes || 30,
    _materials_attachment_ids: materialIds?.length ? materialIds : null,
    _recurrence_rule: null,
  });

  const patch = {
    work_spec: workSpec,
    subject_id: subjectId || null,
    requires_submission_home: !!requireParentApproval,
    curriculum_unit_title: unitTitle?.trim() || null,
    lesson: lessonLabel?.trim() || null,
    curriculum_lesson_id: curriculumLessonId || null,
  };

  const { data: updated, error: patchError } = await supabase
    .from('events')
    .update(patch)
    .eq('id', event.id)
    .select('*')
    .single();
  if (patchError) throw patchError;

  const finalEvent = updated || { ...event, ...patch };

  if (!isDraft) {
    const { data: { user } } = await supabase.auth.getUser();
    await ensureAssignmentsForEvent({
      familyId: userFamilyId,
      event: finalEvent,
      childIds,
      workSpec,
      userId: user?.id || null,
    });

    if (linkedLearningDayEventId) {
      const learningDayId = String(linkedLearningDayEventId);
      const assignmentEventId = String(finalEvent.id);
      const sessionDueYmd = dueDate ? toYmd(dueDate) : toYmd(new Date());
      const { data: assignmentRows, error: assignmentQueryError } = await supabase
        .from('assignments')
        .select('id, linked_event_ids')
        .eq('family_id', userFamilyId)
        .contains('linked_event_ids', [assignmentEventId]);
      if (!assignmentQueryError && Array.isArray(assignmentRows)) {
        await Promise.all(
          assignmentRows.map((row) => {
            const patch = {
              linked_event_ids: [assignmentEventId, learningDayId],
              due_date: sessionDueYmd,
              updated_at: new Date().toISOString(),
            };
            if (unitId) patch.related_syllabus_unit = String(unitId);
            return supabase.from('assignments').update(patch).eq('id', row.id);
          }),
        );
      }
    }

    if (unitId && !curriculumLessonId) {
      const eventIdStr = String(finalEvent.id);
      const { data: assignmentRows, error: assignmentQueryError } = await supabase
        .from('assignments')
        .select('id')
        .eq('family_id', userFamilyId)
        .contains('linked_event_ids', [eventIdStr]);
      if (!assignmentQueryError && Array.isArray(assignmentRows) && assignmentRows.length > 0) {
        await Promise.all(
          assignmentRows.map((row) =>
            supabase
              .from('assignments')
              .update({
                related_syllabus_unit: String(unitId),
                updated_at: new Date().toISOString(),
              })
              .eq('id', row.id),
          ),
        );
      }
    }
  }

  dispatchRefresh();
  dispatchSubjectRefresh(subjectId);
  return finalEvent;
}

export const RECURRENCE_WEEKDAY_OPTIONS = [
  { value: 0, label: 'Sun', rrule: 'SU' },
  { value: 1, label: 'Mon', rrule: 'MO' },
  { value: 2, label: 'Tue', rrule: 'TU' },
  { value: 3, label: 'Wed', rrule: 'WE' },
  { value: 4, label: 'Thu', rrule: 'TH' },
  { value: 5, label: 'Fri', rrule: 'FR' },
  { value: 6, label: 'Sat', rrule: 'SA' },
];

export function buildEventRecurrenceRule({
  recurrenceType = 'weekly',
  recurrenceWeekdays = [],
  recurrenceEndType = 'never',
  recurrenceEndAfter = null,
  recurrenceEndDate = null,
  startDate = null,
  interval = 1,
}) {
  const type = String(recurrenceType || 'weekly').toLowerCase();
  const rule = {
    frequency: type.toUpperCase(),
    interval: Math.max(1, Number(interval) || 1),
  };

  if (type === 'weekly') {
    const fallbackWeekday = startDate instanceof Date ? startDate.getDay() : new Date().getDay();
    const days = Array.isArray(recurrenceWeekdays) && recurrenceWeekdays.length > 0
      ? recurrenceWeekdays
      : [fallbackWeekday];
    rule.byweekday = days
      .map((day) => RECURRENCE_WEEKDAY_OPTIONS.find((opt) => opt.value === Number(day))?.rrule)
      .filter(Boolean);
  }

  if (recurrenceEndType === 'after') {
    const count = Number(recurrenceEndAfter);
    if (Number.isFinite(count) && count > 0) rule.count = count;
  } else if (recurrenceEndType === 'on' && recurrenceEndDate) {
    rule.until = toYmd(recurrenceEndDate);
  }

  return rule;
}

export function buildWeeklyRecurrenceRule(date, interval = 1) {
  if (!(date instanceof Date)) return null;
  return buildEventRecurrenceRule({
    recurrenceType: 'weekly',
    recurrenceWeekdays: [date.getDay()],
    recurrenceEndType: 'never',
    startDate: date,
    interval,
  });
}
