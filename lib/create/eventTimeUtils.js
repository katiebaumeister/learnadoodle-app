...
export function computeEventTimes({
  ...
  const endBaseDate = endDate instanceof Date && !Number.isNaN(endDate.getTime())
    ? new Date(endDate)
    : new Date(baseDate);
  endBaseDate.setHours(0, 0, 0, 0);
  const isMultiDay = endBaseDate.getTime() > baseDate.getTime();

  if (allDay) {
    const start = new Date(baseDate);
    start.setHours(0, 0, 0, 0);
    const end = new Date(endBaseDate);
  ...
  if (!normalizedStart) {
    if (allowOptionalTime) {
      ...
      const minutes = Math.round((end.getTime() - start.getTime()) / (1000 * 60));
      return { start, end, minutes, isFlexible: true };
    }
  ...
  let end = normalizedEnd ? applyTimeToDate(endBaseDate, normalizedEnd) : null;
  if (!end || end <= start) {
    end = isMultiDay
      ? (() => { const e = new Date(endBaseDate); e.setHours(23, 59, 59, 999); return e; })()
