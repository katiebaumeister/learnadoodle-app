...
export function sanitizeLegacyPlanYearView(view) {
  ...
  if (raw === 'plan-year' || raw === 'edit-year') return PLANNER_DEFAULT_CALENDAR_VIEW;
  return raw || PLANNER_DEFAULT_CALENDAR_VIEW;
}
...
/** Opens the standalone Day off create modal (no School Year Settings backdrop). */
export function dispatchOpenDayOffModal(defaultDate = null, schoolYearLabel = null) {
  if (Platform.OS !== 'web' || typeof window === 'undefined') return false;
