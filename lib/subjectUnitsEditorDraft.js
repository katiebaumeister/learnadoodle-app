...
export function unitsFromCurriculumDraft(draft) {
  ...
  return units.map((unit, unitIndex) => {
    const unitId = idFromDraftTempId(unit?.temp_id, 'u');
    return {
      id: isPersistedCurriculumId(unitId) ? unitId : (unitId ? unitId : `idx-${unitIndex}`),
      title: String(unit?.title || '').trim() || `Unit ${unitIndex + 1}`,
      lessons: (unit?.lessons || []).map((lesson, lessonIndex) => {
        const lessonId = idFromDraftTempId(lesson?.temp_id, 'l');
        return {
          id: isPersistedCurriculumId(lessonId) ? lessonId : (lessonId || null),
          title: String(lesson?.title || '').trim() || `Lesson ${lessonIndex + 1}`,
        };
      }),
    };
