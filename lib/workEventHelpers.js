...
import { normalizeMultipleChoice } from './studentResponseTypes';
...
export function getWorkStatusLabel(assignment) {
  ...
  if (review === 'approved' || status === 'accepted' || status === 'reviewed') return 'Reviewed';
  ...
  if (status === 'assigned' || status === 'not_started') return 'Assigned';
  return 'Assigned';
}
...
export function normalizeQuizQuestions(workSpec) {
  ...
  return raw
    .map((row, index) => {
      ...
      if (!id) return null;
      const questionType = row.question_type === 'multiple_choice' ? 'multiple_choice' : 'short_answer';
      let options = [];
      if (questionType === 'multiple_choice' && Array.isArray(row.options)) {
        options = row.options
          .map((opt, optIndex) => {
            if (!opt || typeof opt !== 'object') return null;
            const optId = String(opt.id || `${id}_o_${optIndex + 1}`).trim();
            if (!optId) return null;
            return { id: optId, text: String(opt.text ?? opt.label ?? '').trim() };
          })
          .filter(Boolean);
      }
      const correct = String(row.correct_option_id || '').trim();
      return {
        id,
        prompt,
        question_type: questionType,
        options,
        correct_option_id: options.some((opt) => opt.id === correct) ? correct : null,
...
export function resolveQuizAnswerRows(workSpec, description) {
  const questions = resolveQuizQuestionsForStudent(workSpec);
  const answers = extractQuizAnswers(description);
  return questions.map((q, index) => {
    const rawAnswer = answers[q.id] || '';
    let answer = rawAnswer;
    if (q.question_type === 'multiple_choice' && Array.isArray(q.options)) {
      const match = q.options.find((row) => String(row.id) === String(rawAnswer));
      if (match) answer = match.text || rawAnswer;
    }
    return {
      id: q.id,
...
export function resolveQuizQuestionsForStudent(workSpec) {
  const spec = workSpec && typeof workSpec === 'object' ? workSpec : {};
  const responseType = String(spec.student_response_type || '').trim();
  if (responseType === 'multiple_choice') {
    const mc = normalizeMultipleChoice(spec.multiple_choice);
    return [{
      id: 'assignment_multiple_choice',
      prompt: mc.prompt,
  ...
  return normalizeQuizQuestions(spec);
}
...
export function resolveStudentSubmissionModes(workSpec, eventType = 'Assignment') {
  ...
  const quizQuestions = resolveQuizQuestionsForStudent(spec);
  const responseType = String(spec.student_response_type || '').trim();
  const quizEnabled = configured
    ? !!methods.quiz
    : responseType === 'multiple_choice' || responseType === 'structured_qa';
  return {
    text: configured ? !!methods.text : true,
    file: configured ? !!methods.file : true,
    photo: !!methods.photo,
    link: !!methods.link,
    quiz: quizEnabled && quizQuestions.length > 0,
    parentCheckoff: !!methods.parent_checkoff,
    quizQuestions,
    responseFormat: spec.response_format === 'long' ? 'long' : 'short',
  };
}
...
export function getStudentSubmissionStatusLabel(assignment) {
  ...
  if (review === 'needs_revision' || review === 'rejected') return 'Needs revision';
  const status = String(assignment.status || '').trim().toLowerCase();
  if (status === 'reviewed' || status === 'accepted' || review === 'approved') return 'Complete';
  if (status === 'submitted' || assignment.submitted_at) return 'Submitted';
  if (status === 'in_progress' || Number(assignment.progress_percent) > 0) return 'In progress';
  if (status === 'draft') return 'Draft';
  if (status === 'assigned' || status === 'not_started') return 'Assigned';
  return 'Assigned';
}
