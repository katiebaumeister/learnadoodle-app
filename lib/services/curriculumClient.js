...
const CURRICULUM_STRUCTURE_FAILURE_BACKOFF_MS = 15000;
const curriculumStructureInFlight = new Map();
const curriculumStructureFailureBackoffUntilByKey = new Map();

export function invalidateSubjectCurriculumStructureCache(
  familyId,
  subjectId,
  academicYearId = null,
) {
  ...
  if (!fid || !sid) return;
  const requestKey = `${fid}:${sid}:${yid}`;
  curriculumStructureInFlight.delete(requestKey);
  curriculumStructureFailureBackoffUntilByKey.delete(requestKey);
}
...
export async function commitParsedDraft(params) {
  try {
    ...
    if (params.create_calendar_events === false) {
...
export function validateManualDraftForCommit(draft) {
  ...
  for (let i = 0; i < draft.units.length; i++) {
    ...
    // Empty units are allowed (classwork can add a unit before lessons).
    const lessons = Array.isArray(u.lessons) ? u.lessons : [];
    for (let j = 0; j < lessons.length; j++) {
      const le = lessons[j];
      if (!((le.title || '').trim())) {
        const unitTitle = (u.title || '').trim() || `Unit ${i + 1}`;
        return `Lesson ${j + 1} in ${unitTitle} must have a title.`;
      }
    }
  }
  return null;
 ...
 * @returns {Promise<{ data?: { units: Array<{ id?: string, title: string, lessons: unknown[] }> }, error?: any }>}
 */
export async function fetchSubjectCurriculumEventsStructure(familyId, subjectId, academicYearId = null) {
  ...
  const yid =
    academicYearId == null || String(academicYearId).trim() === ''
      ...
      : String(academicYearId).trim();
  if (!fid || !sid) {
    const err = new Error('familyId and subjectId are required');
    err.status = 400;
    return { data: null, error: err };
  }
  const requestKey = `${fid}:${sid}:${yid}`;
  const now = Date.now();
  const backoffUntil = curriculumStructureFailureBackoffUntilByKey.get(requestKey) || 0;
  if (backoffUntil > now) {
    const err = new Error('Curriculum structure temporarily unavailable. Retrying shortly.');
    err.status = 503;
    return { data: null, error: err };
  }
  const inFlight = curriculumStructureInFlight.get(requestKey);
  ...
  const requestPromise = (async () => {
    try {
      ...
      if (yid !== 'null') q.set('academic_year_id', yid);
      const result = await apiRequest(`/api/curriculum/subject-events-structure?${q.toString()}`, {
        method: 'GET',
      });
      if (result?.error) {
        curriculumStructureFailureBackoffUntilByKey.set(
          requestKey,
          Date.now() + CURRICULUM_STRUCTURE_FAILURE_BACKOFF_MS
        );
      } else {
        curriculumStructureFailureBackoffUntilByKey.delete(requestKey);
      }
  ...
  curriculumStructureInFlight.set(requestKey, requestPromise);
