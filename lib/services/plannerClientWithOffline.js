...
function notifyEventRemoved(eventId) {
  if (typeof window === 'undefined') return;
  const safeEventId = String(eventId || '').trim();
  if (!safeEventId) return;
  try {
    window.dispatchEvent(new CustomEvent('eventDeleted', {
      detail: { eventId: safeEventId, source: 'plannerClientWithOffline' },
    }));
    window.dispatchEvent(new CustomEvent('refreshSubjects', {
      detail: { removedEventIds: [safeEventId] },
    }));
...
export async function rescheduleEvent(eventId, newStartAt, newEndAt, origin = 'drag_drop', reason = 'manual move', familyId) {
  try {
    ...
    if (typeof navigator !== 'undefined' && navigator.onLine) {
      ...
      if (data) {
        serverEvent = data?.event && typeof data.event === 'object' ? data.event : data;
        await offlineStorage.storeEvent({ ...data, family_id: familyId || currentEvent.family_id }, { sync_status: 'synced' });
        }
    }

    return { data: serverEvent || updatedEvent, error: null };
  } catch (err) {
    // Fallback to API
    return await apiRescheduleEvent(eventId, newStartAt, newEndAt, origin, reason);
  }
...
/**
 * Create event via Supabase RPC only (no /api/events, no optimistic sync).
 * Use for Doodle/assistant-created events so creation persists and calendar refresh shows the event.
 * @param {{ title, start_ts, end_ts, event_type, child_id?, ... }} eventData - flat event fields
 * @param {string} familyId
 * @param {string[]} childIds - one event created per child
 * @returns {{ data: Array<{ id }>, error: Error | null }}
 */
export async function createEventViaSupabaseRpc(eventData, familyId, childIds) {
  const ids = Array.isArray(childIds) && childIds.length ? childIds : [eventData.child_id].filter(Boolean);
  const created = [];
  for (const cid of ids) {
    const rpcParams = {
      ...
      _event_type: eventData.event_type ?? 'Lesson',
    ...
    const { data: rpcData, error } = await supabase.rpc('create_task_event', rpcParams);
    if (error) {
      console.warn('[plannerClientWithOffline] create_task_event RPC error:', error);
      return { data: created, error };
    }
    if (rpcData?.ok && rpcData?.id) created.push({ id: rpcData.id });
    else if (rpcData?.event?.id) created.push({ id: rpcData.event.id });
  }
...
export async function updateEvent(eventId, updates, familyId) {
  try {
    // The optimistic path requires the row to already live in local offline storage.
    // Recurring-series instances and any event loaded fresh from the server
    // (get_month_view / get_week_view) are NOT cached locally, so going straight to
    // updateEventOptimistic would throw "Event not found". Detect that case and write
    // directly to the DB instead (mirrors the rescheduleEvent fallback).
    const cached = await offlineStorage.getEvent(eventId).catch(() => null);
    if (!cached) {
      let query = supabase
        .from('events')
        .update(updates)
      ...
      try {
        ...
        // Caching the fresh row is best-effort; the DB write already succeeded.
      }
