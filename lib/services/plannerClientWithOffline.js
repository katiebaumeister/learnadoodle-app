...
export async function rescheduleEvent(eventId, newStartAt, newEndAt, origin = 'drag_drop', reason = 'manual move', familyId) {
  try {
    ...
    if (typeof navigator !== 'undefined' && navigator.onLine) {
      apiRescheduleEvent(eventId, newStartAt, newEndAt, origin, reason).then(({ data, error }) => {
        if (!error && data) {
          // Update with server response
          offlineStorage.storeEvent({ ...data, family_id: familyId || currentEvent.family_id }, { sync_status: 'synced' });
        }
      }).catch(err => {
        console.error('[plannerClientWithOffline] Background reschedule error:', err);
      });
    }

    return { data: updatedEvent, error: null };
  } catch (err) {
    console.error('[plannerClientWithOffline] Reschedule error:', err);
    // Fallback to API
    return await apiRescheduleEvent(eventId, newStartAt, newEndAt, origin, reason);
  }
}
  ...
  try {
    ...
    if (typeof navigator !== 'undefined' && navigator.onLine) {
      supabase.rpc('create_task_event', {
        ...
        console.error('[plannerClientWithOffline] Background create error:', err);
      });
    }

    return { data: optimisticEvent, error: null };
  } catch (err) {
    console.error('[plannerClientWithOffline] Create event error:', err);
...
export async function deleteEvent(eventId, familyId) {
  try {
    ...
    // Also try to delete via API in background if online
    if (typeof navigator !== 'undefined' && navigator.onLine) {
      ...
      if (error) {
        console.error('[plannerClientWithOffline] Delete error:', error);
    ...
    console.error('[plannerClientWithOffline] Delete event error:', err);
    return { data: null, error: err };
  }
}
...
export async function updateEvent(eventId, updates, familyId) {
  try {
    ...
    console.error('[plannerClientWithOffline] Update event error:', err);
    return { data: null, error: err };
  }
}
