...
/** Enrich author profiles with family member emails (API has auth email when profiles.email is empty). */
export function mergeFamilyMemberProfiles(profileMap, familyMembers = []) {
  const next = profileMap instanceof Map ? new Map(profileMap) : new Map();
  (familyMembers || []).forEach((member) => {
    const userId = member?.user_id || member?.userId;
    if (!userId) return;
    const key = String(userId);
    const existing = next.get(key) || { id: userId };
    const email = member?.email || existing.email || null;
    const memberName = member?.name || member?.display_name || null;
    next.set(key, {
      ...existing,
      email,
      name: existing.name || memberName || null,
...
/** Parent/adult author avatar from profiles.avatar_url (prof1–prof10), with stable hash fallback. */
export function avatarSourceForUser(profileMap, userId) {
  const profile = profileMap?.get?.(String(userId));
  const raw = profile?.avatarUrl;
  if (raw) {
    const src = resolveBundledAvatarSource(raw);
    if (src) return src;
  }
  const id = String(userId || '');
  if (!id) return resolveBundledAvatarSource('prof1');
  let hash = 0;
  for (let i = 0; i < id.length; i += 1) {
    hash = (hash + id.charCodeAt(i)) % 10;
  }
...
export async function familySystemBulletinPostExists(familyId, systemKind) {
  if (!familyId || !systemKind) return false;
  const { data, error } = await supabase
    .from('family_bulletin_posts')
    .select('id')
    .eq('family_id', familyId)
    ...
    .maybeSingle();
  if (error) {
    console.warn('[bulletinClient] familySystemBulletinPostExists:', error.message);
    return false;
  }
...
export async function updateBulletinPost({
  ...
  // Omit materialIds to leave attachments unchanged (chat edits often only change body/audience).
