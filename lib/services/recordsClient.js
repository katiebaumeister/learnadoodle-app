...
export async function getPortfolioUploads(childId) {
  ...
  if (error) {
    ...
    console.error('[getPortfolioUploads] Error:', {
...
export async function getComplianceStatus(familyId, childIds, dateRange) {
  ...
  try {
    ...
    if (error?.status === 404) {
      ...
      console.warn('Error loading compliance status, using fallback:', error.message);
    }
  } catch (err) {
    // Network error or other exception - use fallback
    console.warn('Compliance status API unavailable, using fallback');
  }
...
export async function getRecordsSummary(familyId, childIds, dateRange) {
  ...
  try {
    ...
    if (error?.status === 404) {
      ...
      console.warn('Error loading records summary, using fallback:', error.message);
    }
  } catch (err) {
    // Network error or other exception - use fallback
    console.warn('Records summary API unavailable, using fallback');
  }
  ...
  try {
    ...
    console.error('Error calculating records summary:', error);
  }
...
export async function getEvidence(familyId, childIds, filters, dateRange) {
  ...
  // TODO: Ensure display_order column exists in uploads table (integer, default 0)
  let query = supabase
    .from('uploads')
    .select('id, storage_path, caption, created_at, subject_id, event_id, child_id, mime, bytes, display_order')
...
export async function getEvidenceById(familyId, evidenceId) {
  try {
    ...
    if (uploadError) {
      // Suppress expected 400/404 errors
      if (shouldSuppressError(uploadError)) {
    ...
    const mapped = {
      id: uploadData.id,
      family_id: uploadData.family_id,
      child_ids: uploadData.child_id ? [uploadData.child_id] : [],
      subject: uploadData.subject_id || null,
      type: getEvidenceTypeFromMime(uploadData.mime),
      title: uploadData.caption || uploadData.storage_path?.split('/').pop() || 'Untitled',
      description: uploadData.description || '',
      tags: uploadData.tags || [],
      uploaded_at: uploadData.created_at,
      mime_type: uploadData.mime,
      url: uploadData.storage_path,
      filename: uploadData.storage_path?.split('/').pop() || 'file',
      bytes: uploadData.bytes || 0,
      syllabus_unit_ids: uploadData.syllabus_unit_ids || [],
      planner_event_ids: uploadData.event_id ? [uploadData.event_id] : [],
      display_order: uploadData.display_order || 0, // TODO: Ensure display_order column exists
    };
...
export async function updateEvidenceMetadata(evidenceId, payload) {
  try {
    ...
    if (payload.title !== undefined) updatePayload.caption = payload.title;
    ...
    if (payload.child_ids !== undefined && payload.child_ids.length > 0) {
      updatePayload.child_id = payload.child_ids[0]; // uploads table has single child_id
    }
    ...
    if (payload.planner_event_ids !== undefined && payload.planner_event_ids.length > 0) {
      updatePayload.event_id = payload.planner_event_ids[0]; // uploads table has single event_id
    }
    // TODO: Ensure display_order column exists in uploads table (integer, default 0)
...
export async function reorderEvidence(evidenceOrder) {
  try {
    ...
    // Fallback to Supabase batch update
    // TODO: Ensure display_order column exists in uploads table (integer, default 0)
...
export async function getCourseDetails(courseId, familyId) {
  try {
    ...
    // Get evidence linked to this syllabus/subject
    const { data: evidence } = await supabase
      .from('uploads')
  ...
  try {
    ...
    if (error) {
      ...
      if (shouldSuppressError(error) ||
        ...
        if (!shouldSuppressError(error)) {
          console.warn('Error querying notes table, trying fallback:', error);
        }
      }
    } else if (data) {
      return data;
