...
export async function createAttendanceLog(payload) {
  try {
    const isManualDayEntry = payload?.event_id == null;
    const baseLookup = supabase
      .from('attendance_records')
      .select('id')
      .limit(1);
    const lookupQuery = isManualDayEntry
      ? baseLookup
          .eq('family_id', payload?.family_id)
          .eq('child_id', payload?.child_id)
          .eq('day_date', payload?.day_date)
          .is('event_id', null)
      : baseLookup
    ...
    // Rare race fallback: another writer inserted between lookup and insert.
    const { data: racedExisting, error: racedLookupError } = await lookupQuery.maybeSingle();
    if (racedLookupError || !racedExisting?.id) {
      return { data: null, error: racedLookupError || error };
    }
    const { data: racedUpdated, error: racedUpdateError } = await supabase
      ...
      .single();
    if (racedUpdateError) {
      return { data: null, error: racedUpdateError };
    }
    return { data: racedUpdated, error: null };
  } catch (err) {
    return { data: null, error: err };
  }
}
