...
function mergeAssignmentAttentionByEventId(rows) {
  ...
  for (const row of rows || []) {
    ...
    const eid = row?.linked_event_ids;
    if (eid == null) continue;
    const ids = Array.isArray(eid) ? eid : typeof eid === 'string' ? (() => { try { const p = JSON.parse(eid); return Array.isArray(p) ? p : []; } catch { return []; } })() : [];
  ...
  try {
    ...
    try {
      let materialsQuery = supabase
        .from('materials')
        .select('id, title, provider_name, provider_url, subject_id, subject_key, tags, mime, storage_path')
        .eq('family_id', familyId)
        .is('deleted_at', null)
        .order('created_at', { ascending: false });
    ...
    if (subjectDetailParentLike) {
      try {
        const assignSelect =
          'id, title, description, child_id, due_date, need_help, help_message_log, linked_event_ids, status, review_status, family_id, related_subject, updated_at, created_at';
        let nhQ = supabase
          .from('assignments')
          .select(assignSelect)
          .eq('family_id', familyId)
        ...
        let asgQ = supabase
          ...
          .limit(80);
        if (session) {
          nhQ = applyChildFilter(nhQ, session, 'child_id');
          asgQ = applyChildFilter(asgQ, session, 'child_id');
        } else if (childId) {
          nhQ = nhQ.eq('child_id', childId);
          asgQ = asgQ.eq('child_id', childId);
        }
        const [{ data: nhRows }, { data: asgRows }] = await Promise.all([nhQ, asgQ]);
        assignmentsNeedingHelp = nhRows || [];
        assignmentsAssignedToStudent = asgRows || [];
      } catch (e) {
