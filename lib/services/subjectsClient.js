...
function computeSubjectProgressCounts(events = [], attendanceRecords = [], sections = []) {
  ...
  if (plannedEvents.length > 0) {
    ...
    return {
      progressCompleted: completedEventIdSet.size,
      progressTotal: plannedEvents.length,
    };
  }
  ...
  if (sections.length > 0) {
    const completedSections = sections.filter((section) => {
      ...
      return new Date(section.suggested_due_ts) < new Date();
    });
    return {
      progressCompleted: completedSections.length,
      progressTotal: sections.length,
    };
  }

  return { progressCompleted: 0, progressTotal: 0 };
}
...
export async function getSubjectsWithOverview(familyId, childId = null, session = null) {
  ...
  try {
    ...
    const enrichedSubjects = filteredSubjects.map(subject => {
      ...
      const { progressCompleted, progressTotal } = computeSubjectProgressCounts(
        subjectEvents,
      ...
      // Calculate status
      // ⚪ Not started — no measurable progress yet
      // 🟡 Needs attention — overdue, stale activity, or pacing behind
      // 🟢 On track — otherwise
      let status = 'on_track';
      ...
      const hasMeasurableProgress =
        (progressPercent != null && progressPercent > 0) || progressCompleted > 0;

      if (!hasMeasurableProgress && (subjectEvents.length === 0 || lastActivity === null)) {
        status = 'not_started';
      } else if (
        overdueCount > 0 || 
...
export async function getSubjectDetail(subjectId, familyId, childId = null, session = null) {
  ...
  try {
    ...
    try {
      const assignSelect =
        'id, title, description, child_id, due_date, start_work_by, need_help, help_message_log, linked_event_ids, linked_evidence_ids, linked_review_attachment_ids, status, review_status, review_feedback, reviewed_at, submitted_at, progress_percent, grade_display, grade_value, family_id, related_subject, related_syllabus_unit, updated_at, created_at';
