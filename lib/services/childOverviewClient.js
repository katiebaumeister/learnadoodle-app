...
async function getTodaysEvents(familyId, childId, date) {
  ...
  try {
    ...
    console.warn('Error fetching today\'s events:', error);
    return [];
  }
}
...
async function getUpcomingKeyDates(familyId, childId, date, limit = 5) {
  ...
  try {
    ...
    console.warn('Error fetching upcoming key dates:', error);
    return [];
  }
}
      ...
      try {
        ...
        if (subjectIds.length > 0) {
          try {
            ...
            console.warn('Error fetching subject names:', err);
          }
        }
        ...
        if (!cancelled) {
          console.error('Error loading child overview:', err);
