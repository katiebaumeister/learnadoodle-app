...
export async function syncAllPending(familyId) {
  ...
  try {
    ...
    for (const item of queue) {
      try {
        // Skip events: app uses Supabase RPC (create_task_event) and get_month_view for calendar;
        // /api/events is not implemented, so syncing would only produce 404s.
        if (item.table_name === 'events') continue;
        // Add other table types as needed (e.g. non-events sync)
      } catch (error) {
        failed++;
      }
    }

    // Skip syncing pending events to /api/events (endpoint not implemented; would 404 repeatedly)
