...
function notifyListeners(status) {
  syncListeners.forEach(callback => {
    try {
      ...
      console.error('[InstantSync] Listener error:', error);
    }
  });
}
...
export async function createEventOptimistic(event, familyId) {
  ...
  if (isOnline()) {
    syncEventInBackground(tempId, optimisticEvent).catch(error => {
      console.error('[InstantSync] Background sync failed:', error);
    });
  }
...
async function syncEventInBackground(eventId, event) {
  try {
    ...
    console.error('[InstantSync] Sync error:', error);
    throw error;
  }
}
...
async function syncDeleteInBackground(eventId, familyId) {
  try {
    ...
    console.error('[InstantSync] Delete sync error:', error);
    throw error;
  }
}
...
export async function syncAllPending(familyId) {
  if (syncInProgress) {
    console.log('[InstantSync] Sync already in progress');
  ...
  try {
    ...
    for (const item of queue) {
      try {
        ...
        console.error(`[InstantSync] Failed to sync item ${item.id}:`, error);
        failed++;
      }
    }
    ...
    for (const event of pendingEvents) {
      if (!event.deleted) {
        try {
          ...
          console.error(`[InstantSync] Failed to sync event ${event.id}:`, error);
          failed++;
        }
      }
    }
    ...
    console.error('[InstantSync] Sync all error:', error);
...
export function initBackgroundSync(familyId, intervalMs = 30000) {
  ...
  if (typeof window !== 'undefined') {
    window.addEventListener('online', () => {
      console.log('[InstantSync] Online, syncing...');
      syncAllPending(familyId).catch(error => {
        console.error('[InstantSync] Auto-sync error:', error);
      });
    });
    ...
    setInterval(() => {
      if (isOnline() && !syncInProgress) {
        syncAllPending(familyId).catch(error => {
          console.error('[InstantSync] Periodic sync error:', error);
        });
      }
    }, intervalMs);
  }
