 ...
 * Export learning log
 */
export async function exportLearningLog(childId, rangeStart, rangeEnd, format = 'pdf') {
  try {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
      throw new Error('Not authenticated');
    ...
    const apiBase = getAPIBase();
    const response = await fetch(`${apiBase}/api/exports/learning-log?${params.toString()}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${session.access_token}`,
      },
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `learning_log_${rangeStart}_${rangeEnd}.${format === 'pdf' ? 'pdf' : 'docx'}`;
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);
 ...
 * Export report card
 */
export async function exportReportCard(childId, term, grades, behaviorComment, format = 'pdf') {
  try {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
      throw new Error('Not authenticated');
    ...
    const apiBase = getAPIBase();
    const response = await fetch(`${apiBase}/api/exports/report-card`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${session.access_token}`,
        'Content-Type': 'application/json',
    ...
    const a = document.createElement('a');
    a.href = url;
    const sanitizedTerm = term.replace(/\s+/g, '-').replace(/[^a-zA-Z0-9-]/g, '').toLowerCase();
    a.download = `report_card_${sanitizedTerm}.${format === 'pdf' ? 'pdf' : 'docx'}`;
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);
