...
import {
  ...
  pickCurrentTermStartFromPlannerSettings,
  setCachedPlannerTermOptions,
} from './plannerSettingsClient';
...
const planEditListPrefetchPromises = new Map();

const yearAttendanceBundleCache = new Map();

function yearAttendanceCacheKey(familyId, yearStartKey, yearEndKey) {
  return `${String(familyId || '').trim()}|${yearStartKey}|${yearEndKey}`;
}

export function getCachedYearAttendanceBundle(familyId, yearStartKey, yearEndKey) {
  if (!familyId || !yearStartKey || !yearEndKey) return null;
  return yearAttendanceBundleCache.get(yearAttendanceCacheKey(familyId, yearStartKey, yearEndKey)) || null;
}

export function setCachedYearAttendanceBundle(familyId, yearStartKey, yearEndKey, bundle = {}) {
  if (!familyId || !yearStartKey || !yearEndKey) return;
  yearAttendanceBundleCache.set(yearAttendanceCacheKey(familyId, yearStartKey, yearEndKey), {
    attendanceRecords: Array.isArray(bundle.attendanceRecords) ? bundle.attendanceRecords : [],
    yearEvents: Array.isArray(bundle.yearEvents) ? bundle.yearEvents : [],
  });
}
...
/** Warm planner List view events (±3/6 months around anchor) so opening List is stable. */
export async function prefetchPlannerListEvents(familyId, anchorDate = new Date()) {
  if (!familyId) return [];
  const base = anchorDate instanceof Date && !Number.isNaN(anchorDate.getTime()) ? anchorDate : new Date();
  const allPastMonths = 3;
  const allFutureMonths = 6;
  const start = new Date(base.getFullYear(), base.getMonth() - allPastMonths, 1, 0, 0, 0, 0);
  const end = new Date(base.getFullYear(), base.getMonth() + allFutureMonths + 1, 0, 23, 59, 59, 999);
  try {
...
export async function prefetchPlannerAttendanceSnapshot(familyId, childrenList = []) {
  if (!isUuidLike(familyId)) return null;
  ...
  try {
    ...
    const [logs, eventsRes, settingsRes] = await Promise.all([
    ...
    const termOptions = buildBulkTermOptionsFromPlannerSettings(settings);
    const todayKey = toLocalYYYYMMDD(new Date());
    const selectedBulkTermIdx = termOptions.findIndex((t) => t.start <= todayKey && todayKey <= t.end);
    const bulkTermStartKey = pickCurrentTermStartFromPlannerSettings(settings, new Date());
    setCachedPlannerTermOptions(familyId, {
      termOptions,
      bulkTermStartKey,
      selectedBulkTermIdx: selectedBulkTermIdx >= 0 ? selectedBulkTermIdx : 0,
    });

    const attendanceRecords = logs || [];
    const yearEvents = eventsRes?.data || [];
    setCachedYearAttendanceBundle(familyId, yearStartKey, yearEndKey, {
      attendanceRecords,
      yearEvents,
    });

    // Also warm the calendar-year bundle for the current calendar year (planner Year view).
    const calendarYear = new Date().getFullYear();
    const calStartKey = `${calendarYear}-01-01`;
    const calEndKey = `${calendarYear}-12-31`;
    if (calStartKey !== yearStartKey || calEndKey !== yearEndKey) {
      try {
        const calFetchStart = dateStringToDate(calStartKey);
        const calFetchEnd = dateStringToDate(calEndKey);
        const [calLogs, calEventsRes] = await Promise.all([
          getAttendanceLogs(familyId, childIds.length ? childIds : null, { start: calStartKey, end: calEndKey }),
          supabase
            .from('events')
            .select('*')
            .eq('family_id', familyId)
            .gte('start_ts', calFetchStart.toISOString())
            .lte('start_ts', calFetchEnd.toISOString())
            .neq('status', 'canceled')
            .is('deleted_at', null)
            .order('start_ts', { ascending: true }),
        ]);
        setCachedYearAttendanceBundle(familyId, calStartKey, calEndKey, {
          attendanceRecords: calLogs || [],
          yearEvents: calEventsRes?.data || [],
        });
      } catch (_) {
        /* non-blocking */
      }
...
/**
 * Warm saved-schedule list caches (academic year rows + instructional time sublines) on initial load
 * so the first open of schedule management shows times without waiting on extra round trips.
 */
export async function prefetchPlanEditListForFamily(familyId) {
  if (!isUuidLike(familyId)) return;
  const existing = planEditListPrefetchPromises.get(familyId);
  ...
  planEditListPrefetchPromises.set(familyId, run);
  try {
    await run;
  } finally {
    planEditListPrefetchPromises.delete(familyId);
  }
}
