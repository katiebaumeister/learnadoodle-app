...
import { getAccessibleChildIds, canAccessChild } from '../queryFilters.js';
  ...
  if (session) {
    ...
    if (accessibleChildIds.length > 0) {
      filtered = filtered.filter((m) => {
        const links = m.material_children || [];
        // New uploads often have no material_children rows until the parent assigns learners.
        // Those rows must still appear in the library for parents (and tutors), not be filtered out.
        if (links.length === 0) {
          return !!(session.role_flags?.isParent || session.role_flags?.isTutor);
        }
        return links.some((mc) => accessibleChildIds.includes(mc.child_id));
      });
...
/**
 * Load calendar events and plan context for a material (primary attachment, multi-attach array,
 * and optional materials.event_id back-reference).
 * @returns {Promise<{ events: Array<{ id: string, title, date_local, start_local, academic_year_id, curriculum_unit_title, unit, lesson, linkRole: 'primary'|'attachment' }>, planYearById: Record<string, string> }>}
 */
export async function getMaterialLinkages(materialId, familyId) {
  const empty = { events: [], planYearById: {} };
  if (!materialId || !familyId) return empty;
  ...
  try {
    ...
    const eventSelect =
      'id, title, start_local, date_local, academic_year_id, curriculum_unit_title, unit, lesson, material_id, materials_attachment_ids';
    ...
    const addEv = (ev, role) => {
      ...
      if (!prev || role === 'primary') {
        byId.set(ev.id, { ...ev, linkRole: role });
      }
    };
    ...
    (primaryRows || []).forEach((ev) => addEv(ev, 'primary'));

    const { data: attachRows, error: attachErr } = await supabase
      .from('events')
      .select(eventSelect)
      .eq('family_id', familyId)
      .contains('materials_attachment_ids', [materialId])
      .is('deleted_at', null);

    if (!attachErr && attachRows?.length) {
      attachRows.forEach((ev) => addEv(ev, 'attachment'));
    }

    const events = [...byId.values()].sort((a, b) => {
      const da = String(a.date_local || '').localeCompare(String(b.date_local || ''));
      if (da !== 0) return da;
      return String(a.start_local || '').localeCompare(String(b.start_local || ''));
    });

    const yearIds = [...new Set(events.map((e) => e.academic_year_id).filter(Boolean))];
    const planYearById = {};
    if (yearIds.length) {
      const { data: years } = await supabase
        ...
        .in('id', yearIds);
      (years || []).forEach((y) => {
        planYearById[y.id] = y.year_name || 'Plan';
      });
    }

    return { events, planYearById };
  } catch (e) {
    console.warn('[getMaterialLinkages]', e);
    return empty;
  }
}
