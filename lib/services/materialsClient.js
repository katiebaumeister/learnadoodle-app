...
import { getAccessibleChildIds } from '../queryFilters';
...
export async function getMaterials(familyId, filters = {}, session = null) {
  ...
  // Filter by child_id or session-based accessible children
  let filtered = data || [];
  ...
  if (session) {
    ...
    if (accessibleChildIds.length > 0) {
      // Filter materials that are linked to accessible children
  ...
  } else if (filters.child_id) {
    // Legacy: filter by single child_id
    filtered = filtered.filter(m => 
