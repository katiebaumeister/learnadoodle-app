...
const deriveRangeDefaultsFromSchoolYearLabel = (schoolYearLabel) => {
  ...
  if (!match) {
    return {
      ...
      default_summer_term_start_date: null,
      default_summer_term_end_date: null,
    };
  }
  ...
  return {
    ...
    default_summer_term_start_date: `${endYear}-06-01`,
    default_summer_term_end_date: `${endYear}-07-31`,
  };
};
...
const coerceRangeDefaultsToRequestedSchoolYear = (settings, schoolYearLabel) => {
  ...
  next.default_summer_term_start_date = keepIfYearMatches(
    settings?.default_summer_term_start_date,
    endYear,
    defaults.default_summer_term_start_date
  );
  next.default_summer_term_end_date = keepIfYearMatches(
    settings?.default_summer_term_end_date,
    endYear,
    defaults.default_summer_term_end_date
  );
...
// Pulls the offending column out of a Postgres/PostgREST "column ... does not exist"
// error so we can strip only that column and keep the rest of the payload.
const extractMissingColumnName = (error) => {
  const msg = String(error?.message || error?.details || '');
  const match = msg.match(/column\s+(?:[\w".]+\.)?"?([a-z0-9_]+)"?\s+does not exist/i);
...
const plannerTermOptionsCache = new Map();

export function buildBulkTermOptionsFromPlannerSettings(settings) {
  if (!settings) return [];
...
export function deriveDefaultBulkTermOptions(refDate = new Date()) {
  const effectiveSchoolYear = resolveSchoolYearLabel(null, refDate);
  const rangeDefaults = deriveRangeDefaultsFromSchoolYearLabel(effectiveSchoolYear);
  return buildBulkTermOptionsFromPlannerSettings(rangeDefaults);
}

export function pickCurrentTermStartFromPlannerSettings(settings, today = new Date()) {
  if (!settings) return null;
  const todayKey = typeof today === 'string'
...
export function getCachedPlannerTermOptions(familyId) {
  const key = String(familyId || '').trim();
  if (!key) return null;
  return plannerTermOptionsCache.get(key) || null;
}

export function setCachedPlannerTermOptions(familyId, payload = {}) {
  const key = String(familyId || '').trim();
  if (!key) return;
  plannerTermOptionsCache.set(key, {
    termOptions: Array.isArray(payload.termOptions) ? payload.termOptions : [],
    bulkTermStartKey: payload.bulkTermStartKey || null,
    selectedBulkTermIdx: Number.isFinite(payload.selectedBulkTermIdx) ? payload.selectedBulkTermIdx : 0,
  });
}
...
export const saveFamilyPlannerSettings = async (familyId, payload, schoolYearLabel = null) => {
  ...
  // Backward compatibility: some deployed DBs may not yet have every optional column
  // (e.g. the summer term range). Strip ONLY the specific missing column and retry so
  // the remaining term dates still persist, instead of dropping all of them wholesale.
  if (isMissingCompatColumnError(error)) {
    let attemptPayload = { ...writePayload };
    let lastError = error;
    let guard = 0;
    while (lastError && isMissingCompatColumnError(lastError) && guard <= COMPAT_OPTIONAL_COLUMNS.length) {
      guard += 1;
      const missingCol = extractMissingColumnName(lastError);
      if (missingCol && missingCol in attemptPayload) {
        delete attemptPayload[missingCol];
      } else {
        // Couldn't identify the column — fall back to stripping all optional columns.
        attemptPayload = stripRangeDefaultColumns(attemptPayload);
      }
      plannerSettingsCompatLog('write:missing_column_retry', {
        familyId,
        schoolYearLabel: effectiveSchoolYear,
        strippedColumn: missingCol || 'all_optional',
      });
    ...
    if (!lastError) return { error: null };
    if (isLegacySchoolYearSchemaError(lastError)) {
      plannerSettingsCompatLog('write:legacy_fallback_after_range_retry', {
        familyId,
        schoolYearLabel: effectiveSchoolYear,
        reason: String(lastError?.message || lastError?.details || '').slice(0, 240),
      });
      const legacyPayload = {
        ...stripRangeDefaultColumns(attemptPayload),
      };
...
export async function syncFamilyHolidayBreakExclusions(familyId, customHolidays, customBreaks, schoolYearLabel = null) {
  ...
  const syncedCustomHolidays = [];
  for (const h of customHolidays || []) {
    if (h.id) {
      const { data, error } = await updateExclusion(h.id, { start_date: h.date, end_date: h.date, label: h.name });
      if (error) return { error };
      syncedCustomHolidays.push({
        id: data?.id || h.id,
        date: sliceIsoDate(data?.start_date) || h.date,
        name: data?.label || h.name || '',
      });
    } else {
      ...
      if (error) return { error };
      syncedCustomHolidays.push({
        id: data?.id || h.id || null,
        date: sliceIsoDate(data?.start_date) || h.date,
        name: data?.label || h.name || '',
      });
    }
  }
  const syncedCustomBreaks = [];
  for (const b of customBreaks || []) {
    if (b.id) {
      const { data, error } = await updateExclusion(b.id, { start_date: b.start, end_date: b.end, label: b.name });
      if (error) return { error };
      syncedCustomBreaks.push({
        id: data?.id || b.id,
        start: sliceIsoDate(data?.start_date) || b.start,
        end: sliceIsoDate(data?.end_date) || b.end,
        name: data?.label || b.name || '',
      });
    } else {
      const { data, error } = await addExclusion({
        family_id: familyId,
  ...
  return { error: null, customHolidays: syncedCustomHolidays, customBreaks: syncedCustomBreaks };
}
