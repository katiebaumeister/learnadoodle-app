...
const stripRangeDefaultColumns = (payload = {}) => {
  const next = { ...payload };
  RANGE_DEFAULT_COLUMNS.forEach((key) => {
    if (key in next) delete next[key];
  });
  return next;
};

const isMissingRangeColumnError = (error) => {
  const msg = String(error?.message || error?.details || '').toLowerCase();
  if (!msg) return false;
  return (
    msg.includes('family_planner_settings')
    && msg.includes('column')
    && RANGE_DEFAULT_COLUMNS.some((col) => msg.includes(col))
  );
};
...
export const saveFamilyPlannerSettings = async (familyId, payload, schoolYearLabel = null) => {
  ...
  if (isMissingRangeColumnError(error)) {
