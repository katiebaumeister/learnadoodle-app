...
export async function updateAssignmentPlacement({
  ...
  const safeUnitId = isPersistedCurriculumId(unitId) ? String(unitId) : null;
  const safeLessonId = isPersistedCurriculumId(lessonId) ? String(lessonId) : null;

  const { error: assignError } = await supabase
    .from('assignments')
    .update({
      related_syllabus_unit: safeUnitId,
  ...
  if (safeLessonId) {
    const patch = buildLessonLinkEventPatch({
