...
const localCompleteEventFallback = async (eventId, minutesOverride = null, options = {}) => {
  try {
    ...
    const chosenChildId = options?.childId || eventRow.child_id || childIds[0] || null;
    if (!chosenChildId) {
...
export async function completeEvent(eventId, minutesOverride = null, options = {}) {
  ...
  try {
    const body = {};
    if (minutesOverride !== null) body.minutes_override = minutesOverride;
    if (options?.childId) body.child_id = options.childId;
    ...
    if (error) {
      ...
      if (shouldTryFallback) {
        ...
        const localFallback = await localCompleteEventFallback(eventId, minutesOverride, options);
...
export async function updateEventStatus(eventId, status, options = {}) {
  ...
  const clearAttendance = options?.clearAttendance !== false;
  const normalizedStatus = String(status || '').trim().toLowerCase();
  const shouldClearAttendance = clearAttendance
