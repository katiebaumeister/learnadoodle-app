  ...
  try {
    ...
    if (error) {
      ...
      if (!shouldSuppressError(error)) {
        console.error('generateDailySuggestions error:', errorMsg, errorCode, errorDetails, errorHint);
      } else {
        // Log suppressed errors with full details - use console.log so it's definitely visible
        console.log('[generateDailySuggestions] ERROR (suppressed):', errorMsg);
        console.log('[generateDailySuggestions] Code:', errorCode);
        console.log('[generateDailySuggestions] Details:', errorDetails);
        console.log('[generateDailySuggestions] Hint:', errorHint);
      }
    ...
    if (!shouldSuppressError(err)) {
      console.error('generateDailySuggestions exception:', err);
    }
...
export async function getActiveSuggestions(familyId, childId = null) {
  try {
    ...
    if (error) {
      if (!shouldSuppressError(error)) {
        console.error('getActiveSuggestions error:', error);
      }
      return { data: null, error };
    }
    ...
    if (!shouldSuppressError(err)) {
      console.error('getActiveSuggestions exception:', err);
    }
    return { data: null, error: err };
  }
}
...
export async function dismissSuggestion(suggestionId) {
  try {
    ...
    if (!shouldSuppressError(err)) {
      console.error('dismissSuggestion exception:', err);
    }
    return { data: null, error: err };
  }
}
