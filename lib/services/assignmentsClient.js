...
export async function createAssignment(assignmentData) {
  try {
    const payload = {
      ...
      start_work_by: assignmentData.start_work_by || null,
      ...
      linked_review_attachment_ids: assignmentData.linked_review_attachment_ids || [],
    ...
    if (error && isMissingColumnError(error, 'start_work_by')) {
      const fallbackPayload = { ...payload };
      delete fallbackPayload.start_work_by;
      ({ data, error } = await supabase
        .from('assignments')
        .insert(fallbackPayload)
        .select()
    ...
    if (error && isMissingColumnError(error, 'linked_review_attachment_ids')) {
      const fallbackPayload = { ...payload };
      delete fallbackPayload.linked_review_attachment_ids;
      ({ data, error } = await supabase
        .from('assignments')
        .insert(fallbackPayload)
        .select()
...
export async function updateAssignment(assignmentId, updates) {
  try {
    ...
    if (error && payload.linked_review_attachment_ids !== undefined && isMissingColumnError(error, 'linked_review_attachment_ids')) {
      const fallbackPayload = { ...payload };
      delete fallbackPayload.linked_review_attachment_ids;
      ({ data, error } = await supabase
        .from('assignments')
