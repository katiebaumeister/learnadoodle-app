...
export async function getTodayInsights(familyId, date, childIds) {
  ...
  try {
    ...
    // If 404, silently return empty array (endpoint doesn't exist yet)
    if (error?.status === 404) {
      return [];
    }
    ...
    // Silently fall back if endpoint doesn't exist (404s are expected)
...
export async function getMissingEvidenceSummary(familyId, childIds, dateRange) {
  try {
    ...
    console.error('Error getting missing evidence summary:', error);
  ...
  const results = await Promise.all(
    dates.map(async (dateStr) => {
      ...
      try {
        ...
        console.error(`Error loading summary for ${dateStr}:`, error);
        return {
          date: dateStr,
...
export async function getHomeTilesSummary(familyId, date, childIds) {
  ...
  try {
    ...
    console.error('Error getting home tiles summary:', error);
    return {
