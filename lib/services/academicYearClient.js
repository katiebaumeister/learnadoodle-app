 ...
 * @param {{ academicYearId: string, limit?: number }} input
 * @returns {Promise<{data: {rows: Array}|null, error: Error|null}>}
 */
export const getFixTargetGapHistory = async ({ academicYearId, limit = 25 }) => {
  ...
  const { data, error } = await apiRequestWithAuthRetry(
    `/api/academic_year/fix_target_gap/history?academic_year_id=${safeAcademicYearId}&limit=${safeLimit}`,
