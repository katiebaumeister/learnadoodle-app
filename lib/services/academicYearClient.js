...
const shouldRetryPlanYearAuth = (error) => error?.status === 401 || error?.status === 403;

const apiRequestWithAuthRetry = async (endpoint, options) => {
  let result = await apiRequest(endpoint, options);
  if (!shouldRetryPlanYearAuth(result?.error)) return result;
  ...
  result = await apiRequest(endpoint, options);
  return result;
};
...
export const getPlanHealth = async (familyId, academicYearId = null) => {
  ...
  const promise = (async () => {
    const { data, error } = await apiRequestWithAuthRetry(url, { method: 'GET' });
    if (error) {
...
export const computeSchedulePotential = async (input) => {
  const { data, error } = await apiRequestWithAuthRetry('/api/academic_year/schedule_potential', {
    method: 'POST',
    body: JSON.stringify(input),
  });
...
export const applyToCalendar = async (input) => {
  ...
  const { data, error } = await apiRequestWithAuthRetry('/api/academic_year/apply_to_calendar', {
    method: 'POST',
    body: JSON.stringify(input),
    timeoutMs: 180000,
  });
...
/**
 * Deterministically fix target gap to 0 by creating/removing placeholder events.
 * @param {Object} input - { academic_year_id, scope, subject_id?, target_kind, target_value, mode, dry_run }
 * @returns {Promise<{data: Object|null, error: Error|null}>}
 */
export const fixTargetGap = async (input) => {
  const { data, error } = await apiRequestWithAuthRetry('/api/academic_year/fix_target_gap', {
    method: 'POST',
    body: JSON.stringify(input),
    timeoutMs: 180000,
  });
...
/**
 * Fetch persisted Fix Gap history entries for one academic year.
 * @param {{ academicYearId: string, limit?: number|null }} input
 * @returns {Promise<{data: {rows: Array}|null, error: Error|null}>}
 */
export const getFixTargetGapHistory = async ({ academicYearId, limit = null }) => {
  const safeAcademicYearId = encodeURIComponent(String(academicYearId || '').trim());
  const safeLimit = Number.isFinite(Number(limit)) ? Math.max(1, Math.min(1000, Number(limit))) : null;
  const limitQuery = safeLimit != null ? `&limit=${safeLimit}` : '';
  const { data, error } = await apiRequestWithAuthRetry(
    `/api/academic_year/fix_target_gap/history?academic_year_id=${safeAcademicYearId}${limitQuery}`,
...
/**
 * Undo most recent fix-gap action using history id or explicit ids.
 * @param {Object} input - { academic_year_id, scope, subject_id?, history_id?, created_event_ids?, removed_event_ids? }
 * @returns {Promise<{data: Object|null, error: Error|null}>}
 */
export const undoFixTargetGap = async (input) => {
  const { data, error } = await apiRequestWithAuthRetry('/api/academic_year/fix_target_gap/undo', {
    method: 'POST',
    body: JSON.stringify(input),
    timeoutMs: 180000,
  });
