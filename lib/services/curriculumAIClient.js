...
export async function parseSyllabusPDF({
  ...
  try {
    ...
    if (rpcError) {
      console.error('ai_parse_syllabus RPC error:', rpcError);
    ...
    if (error) {
      console.error('parse-syllabus-enhanced API error:', error);
      return { data: null, error };
    }

    return { data: { ...data, syllabus_id: syllabusId, upload_id: uploadId }, error: null };
  } catch (err) {
    console.error('parseSyllabusPDF exception:', err);
...
export async function getParsedSyllabus(syllabusId) {
  try {
    ...
    console.error('getParsedSyllabus exception:', err);
    return { data: null, error: err };
  }
}
...
export async function generatePacing(syllabusId, startDate, endDate) {
  try {
    ...
    console.error('generatePacing exception:', err);
    return { data: null, error: err };
  }
}
...
export async function linkEvidenceToUnit(syllabusId, sectionId, evidenceIds) {
  try {
    ...
    console.error('linkEvidenceToUnit exception:', err);
    return { data: null, error: err };
  }
}
...
export async function generateUnitSummary(sectionId) {
  try {
    ...
    console.error('generateUnitSummary exception:', err);
    return { data: null, error: err };
  }
}
