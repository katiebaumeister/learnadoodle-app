...
export const STUDENT_RESPONSE_NO_RESPONSE = 'no_response';

export const STUDENT_RESPONSE_TYPES = [
  { id: STUDENT_RESPONSE_NO_RESPONSE, label: 'No response' },
  { id: 'structured_qa', label: 'Structured Q&A' },
  { id: 'attachment', label: 'Attachment' },
];

export function parseStudentResponseType(value) {
  if (value == null || value === '') return STUDENT_RESPONSE_NO_RESPONSE;
...
export function normalizeStudentResponseType(value) {
  return parseStudentResponseType(value) || STUDENT_RESPONSE_NO_RESPONSE;
}

export function studentResponseTypeLabel(id) {
  const parsed = parseStudentResponseType(id);
  return STUDENT_RESPONSE_TYPES.find((row) => row.id === parsed)?.label || 'No response';
}

export function studentResponseTypeShowsExtraEditor(type) {
  const parsed = parseStudentResponseType(type);
  return parsed === 'structured_qa' || parsed === 'attachment';
}
...
export function defaultSubmissionMethodsForStudentResponseType(responseType) {
  ...
  if (!type || type === STUDENT_RESPONSE_NO_RESPONSE) return { ...none };
...
export function buildWorkSpecForStudentResponseType(responseType, prev = {}) {
  const type = parseStudentResponseType(responseType);
  if (!type || type === STUDENT_RESPONSE_NO_RESPONSE) {
    return {
      ...prev,
      student_response_type: STUDENT_RESPONSE_NO_RESPONSE,
      submission_methods: defaultSubmissionMethodsForStudentResponseType(STUDENT_RESPONSE_NO_RESPONSE),
