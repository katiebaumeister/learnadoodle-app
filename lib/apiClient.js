...
/** Base URL for the app (sign-in/accept flow). Use when landing is on learnadoodle.com and CTA goes to app. */
export const getAppBase = () => {
  if (typeof window !== 'undefined' && (window.location.hostname === 'learnadoodle.com' || window.location.hostname === 'www.learnadoodle.com')) {
    return LEARNADOODLE_APP_ORIGIN;
  }
  return typeof window !== 'undefined' ? window.location.origin : LEARNADOODLE_APP_ORIGIN;
};
...
export const getAPIBase = () => {
  if (typeof window !== 'undefined') {
    ...
    // Dev: never use frontend origin (e.g. 8081) as API base — backend is on 8001
    if (base.startsWith('http://localhost:') && (base.includes(':8081') || base.endsWith('8081'))) {
      base = 'http://localhost:8001';
    }
    // Runtime fallback: when served from learnadoodle.com, use api.learnadoodle.com (fixes Vercel builds that didn't get env)
    if ((!base || base === window.location.origin) && (window.location.hostname === 'learnadoodle.com' || window.location.hostname === 'www.learnadoodle.com')) {
...
export const apiRequest = async (endpoint, options = {}) => {
  ...
  // Debug: log the API URL (curriculum and academic_year) so we can confirm backend is used
  if (process.env.NODE_ENV === 'development' && (endpoint.includes('curriculum') || endpoint.includes('academic_year'))) {
    console.log('[apiClient] Request URL:', url, '| API_BASE:', API_BASE);
  }
...
export const getEvent = async (eventId) => {
  try {
    ...
    console.log('[getEvent] Loading event data:', {
      eventId,
    ...
    if (eventData && effectiveChildId) {
      ...
      if (!childError && childData) {
        ...
        console.log('[getEvent] Loaded child:', childData);
      } else {
        console.warn('[getEvent] Failed to load child:', childError);
      }
    } else {
      console.log('[getEvent] No effective child ID found - child_id:', eventData?.child_id, 'child_ids:', eventData?.child_ids);
    }
...
/**
 * Onboarding: get status for resume (default_planning_mode, has_children, has_subjects, onboarding_completed)
