...
export const callRPC = async (rpcName, params = {}) => {
  try {
    ...
    if (!shouldSuppressError(err)) {
      console.error(`RPC ${rpcName} exception:`, err);
    }
...
export const apiRequest = async (endpoint, options = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (!shouldSuppress) {
        console.error(`apiRequest HTTP ${response.status}:`, errorMessage);
      }
...
export const getEvent = async (eventId) => {
  try {
    ...
    // Fetch child separately if child_id exists
    if (eventData && eventData.child_id) {
      const { data: childData, error: childError } = await supabase
        .from('children')
        // Use first_name only; some databases don't have a generic name column
        .select('id, first_name')
        .eq('id', eventData.child_id)
        .single();
...
export const createBlackout = async (params) => {
  ...
  console.warn('=== [createBlackout] FUNCTION CALLED ===');
  console.warn('[createBlackout] params:', JSON.stringify(params, null, 2));
  console.error('[createBlackout] Starting - ERROR LEVEL LOG FOR VISIBILITY');
  ...
  try {
    console.warn('[createBlackout] Creating blackout:', {
      familyId: params.familyId,
      childId: params.childId,
      startsOn: startsOnFormatted,
      endsOn: endsOnFormatted,
      reason: params.reason,
    });
    
    // Insert blackout period directly into Supabase
    console.warn('[createBlackout] About to insert into blackout_periods table...');
    const insertPayload = {
    ...
    console.warn('[createBlackout] Insert payload:', JSON.stringify(insertPayload, null, 2));
    ...
    console.warn('[createBlackout] Insert result:', {
      hasData: !!blackout,
      hasError: !!blackoutError,
      blackout: blackout ? JSON.stringify(blackout, null, 2) : null,
      error: blackoutError ? JSON.stringify(blackoutError, null, 2) : null,
    });

    if (blackoutError) {
      console.error('🚨🚨🚨 [createBlackout] ❌ ERROR creating blackout:', blackoutError);
      console.error('🚨🚨🚨 [createBlackout] Error details:', JSON.stringify(blackoutError, null, 2));
      console.error('🚨🚨🚨 [createBlackout] Error code:', blackoutError.code);
      console.error('🚨🚨🚨 [createBlackout] Error message:', blackoutError.message);
      console.error('🚨🚨🚨 [createBlackout] Error hint:', blackoutError.hint);
      return { data: null, error: blackoutError };
    }
    
    if (!blackout) {
      console.error('🚨🚨🚨 [createBlackout] ❌ CRITICAL: Insert succeeded but no data returned!');
      return { data: null, error: new Error('Blackout insert succeeded but returned no data') };
    }
    
    console.log('[createBlackout] Blackout created successfully:', blackout);
    ...
    console.log('[createBlackout] Immediate read-back verification:', {
      found: !!verifyRead,
      blackout: verifyRead,
    ...
    console.log('[createBlackout] Creating overrides for dates from', startsOnFormatted, 'to', endsOnFormatted);
    
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      const dateStr = d.toISOString().split('T')[0];
      
      // First, check if override already exists and deactivate any conflicting ones
      const { data: existing } = await supabase
        .from('schedule_overrides')
        .select('id, override_kind')
        .eq('scope_type', params.childId ? 'child' : 'family')
        .eq('scope_id', params.childId || params.familyId)
        .eq('date', dateStr)
        .eq('is_active', true)
        .maybeSingle();

      console.log(`[createBlackout] Checking override for ${dateStr}:`, existing);

      // If there's an existing override with different kind, deactivate it first
      if (existing && existing.override_kind !== 'day_off') {
        console.log(`[createBlackout] Deactivating existing override for ${dateStr}`);
        const { error: deactivateError } = await supabase
          .from('schedule_overrides')
          .update({ is_active: false })
          .eq('id', existing.id);
        if (deactivateError) {
          console.error(`[createBlackout] Error deactivating override for ${dateStr}:`, deactivateError);
        }
      }

      // If override already exists with day_off, skip upsert to avoid 409
      if (existing && existing.override_kind === 'day_off') {
        console.log(`[createBlackout] Override already exists for ${dateStr}, skipping`);
        overrides.push(dateStr);
        continue;
      }

      // Now upsert the day_off override - wrap in try/catch to handle 409 gracefully
      try {
        ...
        if (overrideError) {
          // If 409, it means the override already exists - that's okay
          if (overrideError.code === '23505' || overrideError.message?.includes('409') || overrideError.message?.includes('conflict')) {
            console.log(`[createBlackout] Override already exists for ${dateStr} (409/conflict - non-fatal)`);
            overrides.push(dateStr); // Count it as created
          } else {
            console.error(`[createBlackout] Failed to create override for ${dateStr}:`, overrideError);
          }
        } else {
          console.log(`[createBlackout] Override created for ${dateStr}`);
          overrides.push(dateStr);
        }
      } catch (overrideErr) {
        // Catch any unexpected errors during upsert
        if (overrideErr.code === '23505' || overrideErr.message?.includes('409') || overrideErr.message?.includes('conflict')) {
          console.log(`[createBlackout] Override conflict for ${dateStr} (non-fatal):`, overrideErr.message);
          overrides.push(dateStr); // Count it as created
        } else {
          console.error(`[createBlackout] Unexpected error creating override for ${dateStr}:`, overrideErr);
        }
      }
    }
    
    console.log('[createBlackout] Created', overrides.length, 'overrides:', overrides);
    ...
    console.log('🚨🚨🚨 [createBlackout] Final verification - querying blackout by ID:', blackout.id);
    ...
    console.log('🚨🚨🚨 [createBlackout] Final verification result:', {
      found: !!finalBlackout,
    ...
    if (!finalBlackout && !finalError) {
      console.error('🚨🚨🚨 [createBlackout] CRITICAL: Blackout was created but cannot be read back! This is likely an RLS issue.');
    }
    
    // Log final summary
    console.log('🚨🚨🚨 [createBlackout] Blackout creation complete:', {
      blackoutId: blackout.id,
      blackout: {
        id: blackout.id,
        family_id: blackout.family_id,
        child_id: blackout.child_id,
        starts_on: blackout.starts_on,
        ends_on: blackout.ends_on,
      },
      overridesCreated: overrides.length,
      dates: overrides,
      canReadBack: !!finalBlackout,
    });
    ...
    console.error('[createBlackout] Exception during blackout creation:', err);
    console.error('[createBlackout] Exception stack:', err.stack);
    console.error('[createBlackout] Exception details:', JSON.stringify(err, null, 2));
    return { data: null, error: err };
  }
};
 ...
 */
      export const proposeReschedule = async (params) => {
        ...
        console.log('[proposeReschedule] Starting API call to:', url);
        console.log('[proposeReschedule] Params:', {
          family_id: params.familyId,
        ...
        const timeoutId = setTimeout(() => {
          console.error('[proposeReschedule] Request timeout after 60 seconds');
          controller.abort();
        }, 60000);
        
        const fetchStartTime = Date.now();
        try {
          console.log('[proposeReschedule] Sending fetch request...');
          const response = await fetch(url, {
            method: 'POST',
          ...
          console.log('[proposeReschedule] Fetch completed in', fetchDuration, 'ms, status:', response.status);
          
          clearTimeout(timeoutId);
          
          if (!response.ok) {
            const errorText = await response.text();
            console.error('[proposeReschedule] HTTP error:', response.status, errorText);
            let errorMessage = `HTTP ${response.status}`;
            try {
          ...
          console.log('[proposeReschedule] Parsing JSON response...');
          const data = await response.json();
          const totalDuration = Date.now() - fetchStartTime;
          console.log('[proposeReschedule] Success! Total duration:', totalDuration, 'ms, data keys:', Object.keys(data || {}));
          return { data, error: null };
        } catch (err) {
          clearTimeout(timeoutId);
          const totalDuration = Date.now() - fetchStartTime;
          console.error('[proposeReschedule] Exception after', totalDuration, 'ms:', err);
...
export const getResumePoint = async (courseId, childId) => {
  try {
    ...
    console.error('[getResumePoint] Error:', err);
    return { data: null, error: err };
  }
};
...
export const updateResumePoint = async (courseId, params, courseType = 'general') => {
  try {
    ...
    console.error('[updateResumePoint] Error:', err);
    return { data: null, error: err };
  }
};
...
export const generateDeepLink = async (courseId, childId, lessonId = null) => {
  try {
    ...
    console.error('[generateDeepLink] Error:', err);
    return { data: null, error: err };
  }
};
...
export const getWeeklyPacket = async (childId, weekStart, format = 'pdf') => {
  try {
    ...
    console.error('[getWeeklyPacket] Error:', err);
    return { data: null, error: err };
  }
};
...
export const createChildInvite = async (childId) => {
  try {
    ...
    console.error('[createChildInvite] Error:', err);
    return { data: null, error: err };
  }
};
...
export const previewChildInvite = async (token) => {
  try {
    ...
    console.error('[previewChildInvite] Error:', err);
    return { data: null, error: err };
  }
};

export const acceptChildInvite = async ({ token, username, email, password }) => {
  try {
    ...
    console.error('[acceptChildInvite] Error:', err);
    return { data: null, error: err };
  }
};
...
export const getLearningStory = async (weekStart = null) => {
  try {
    ...
    console.error('[getLearningStory] Error:', err);
    return { data: null, error: err };
  }
};
...
export const getParentWins = async (weeks = 4) => {
  try {
    ...
    console.error('[getParentWins] Error:', err);
    return { data: null, error: err };
  }
};
...
export const getReadinessMeter = async (childId) => {
  try {
    ...
    console.error('[getReadinessMeter] Error:', err);
    return { data: null, error: err };
  }
};
...
export const getAssuranceCard = async (weekStart = null) => {
  try {
    ...
    console.error('[getAssuranceCard] Error:', err);
    return { data: null, error: err };
  }
};
...
export const getReassuranceMessage = async (childId, context = 'general') => {
  try {
    ...
    console.error('[getReassuranceMessage] Error:', err);
    return { data: null, error: err };
  }
};
...
export const getPacingPrediction = async (childId, subjectId = null) => {
  try {
    ...
    console.error('[getPacingPrediction] Error:', err);
    return { data: null, error: err };
  }
};
...
export const proposeTutorPlan = async (proposal) => {
  try {
    ...
    console.error('[proposeTutorPlan] Error:', err);
    return { data: null, error: err };
  }
};
...
export const logTutorOutcome = async (outcome) => {
  try {
    ...
    console.error('[logTutorOutcome] Error:', err);
    return { data: null, error: err };
  }
};
...
export const getInsights = async (familyId, selectedChildren, dateRange) => {
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404 || response.status === 422 || response.status === 500) {
        if (response.status === 404) {
          console.warn('[getInsights] Endpoint not found (404) - returning empty insights array');
        }
        return { data: [], error: null };
      }
    ...
    console.warn('[getInsights] Exception:', error);
    return { data: [], error: null };
  }
};
