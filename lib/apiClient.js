...
/** Avoid hanging forever on Supabase auth lock contention (multi-tab / slow storage). */
const AUTH_SESSION_READ_TIMEOUT_MS = 8000;

export async function getSupabaseAuthSession({ timeoutMs = AUTH_SESSION_READ_TIMEOUT_MS } = {}) {
  try {
...
export const getAPIBase = () => {
  if (typeof window !== 'undefined') {
    ...
    // Dev: the web/Expo dev server (8081, 19006, etc.) can't serve FastAPI routes,
    // so redirect ONLY those known dev-server ports to the API port. Never rewrite an
    // explicitly-configured API URL (e.g. http://localhost:8001) to a different port —
    // doing so previously clobbered a correct base when REACT_APP_API_PORT disagreed.
    try {
      const u = new URL(base);
      const devHosts = new Set(['localhost', '127.0.0.1']);
      const webDevPorts = new Set(['8081', '8082', '19000', '19001', '19006', '3000']);
      const apiPort =
        (typeof process !== 'undefined' && process.env && process.env.REACT_APP_API_PORT) || '8001';
      if (devHosts.has(u.hostname) && u.port && webDevPorts.has(u.port) && u.port !== apiPort) {
...
export const apiRequest = async (endpoint, options = {}) => {
  ...
  // Get Supabase session token for authentication (bounded — never block API on auth lock)
  ...
  try {
    ...
    // Any successful mutation can change family membership/derived data — drop the
    // cached members + /api/me snapshots so the next read re-fetches fresh.
    ...
    if (err.name === 'AbortError') {
      ...
      } else if (endpoint.includes('permanent_delete')) {
        errorMessage =
          'This is taking longer than expected. The deletion may still be completing in the background — refresh in a moment to check before trying again.';
      } else {
        errorMessage =
          timeoutMs > 45000
...
// `/api/me` is fetched on initial login by SessionContext, WebLayout, and
// WebContent (and a few settings panels). Coalesce concurrent calls and briefly
// cache so the login preload fires one request instead of several.
const ME_CACHE_TTL_MS = 10000;
let _meCache = { ts: 0, data: null };
let _meInFlight = null;

export const clearMeCache = () => {
  _meCache = { ts: 0, data: null };
  _meInFlight = null;
};
...
export const getMe = async ({ force = false } = {}) => {
  ...
  if (!force) {
    ...
    if (_meCache.data && now - _meCache.ts < ME_CACHE_TTL_MS) {
      return { data: _meCache.data, error: null };
    }
  }
  const inFlight = (async () => {
    try {
      ...
      if (!res.error && res.data) {
        _meCache = { ts: Date.now(), data: res.data };
      }
      return res;
    } catch (err) {
      return { data: null, error: err };
...
// `/api/family/members` is fetched by many components at once on the home screen
// (WebLayout, SessionContext, the messages pane, the notification rail, bulletin
// cache, etc.). Without coalescing, a single page load fired ~8 identical backend
// requests in parallel; if the backend was momentarily slow this stacked into a
// multi-minute stall. Share one in-flight request and briefly cache the result.
const FAMILY_MEMBERS_CACHE_TTL_MS = 10000;
let _familyMembersCache = { ts: 0, data: null };
let _familyMembersInFlight = null;

export const clearFamilyMembersCache = () => {
  _familyMembersCache = { ts: 0, data: null };
  _familyMembersInFlight = null;
};

export const getFamilyMembers = async ({ force = false } = {}) => {
  const now = Date.now();
  if (!force) {
    if (_familyMembersInFlight) return _familyMembersInFlight;
    if (_familyMembersCache.data && now - _familyMembersCache.ts < FAMILY_MEMBERS_CACHE_TTL_MS) {
      return { data: _familyMembersCache.data, error: null };
    }
  }
  const inFlight = (async () => {
    try {
      const res = await apiRequest('/api/family/members', { method: 'GET' });
      if (!res.error && res.data) {
        _familyMembersCache = { ts: Date.now(), data: res.data };
      }
      return res;
    } catch (err) {
      return { data: null, error: err };
  ...
  _familyMembersInFlight = inFlight;
  return inFlight;
};
...
export const permanentDeleteChild = async ({ childId, confirmName }) => {
  try {
    return await apiRequest('/api/family/child/permanent_delete', {
      ...
      // Cascades across many tables + Supabase auth-admin; allow more headroom than
      // the 30s default so large accounts don't surface a false timeout.
      timeoutMs: 60000,
    });
  } catch (err) {
    return { data: null, error: err };
  ...
  try {
    return await apiRequest('/api/family/invite', {
      ...
      body: JSON.stringify({
        ...
        parent_name: params.parent_name || null,
        invited_avatar_url: params.invited_avatar_url || null,
        guest_member_id: params.guest_member_id || null,
...
export const createGuestMember = async (payload) => {
  try {
    return await apiRequest('/api/family/guest_members', {
      method: 'POST',
...
export const updateGuestMember = async (guestId, payload) => {
  try {
    return await apiRequest(`/api/family/guest_members/${guestId}`, {
      method: 'PATCH',
      body: JSON.stringify(payload),
    });
  } catch (err) {
...
export const deleteGuestMember = async (guestId) => {
  try {
    return await apiRequest(`/api/family/guest_members/${guestId}`, {
      method: 'DELETE',
    });
...
/**
 * Save family feature settings (workspace toggles).
 * @param {string} familyId
 * @param {Object} featureSettings - { learningAreas, assignments, materials, attendance, grades, complianceRecords }
 */
export const saveFamilyFeatureSettings = async (familyId, featureSettings) => {
  try {
    const { supabase } = await import('./supabase');
    const { error } = await supabase
      .from('family')
      .update({ feature_settings: featureSettings })
      .eq('id', familyId);
...
/**
 * Onboarding: save parent display name + prof avatar (prof1–prof10).
 * @param {Object} payload - { display_name, avatar_url }
 */
export const saveOnboardingParentProfile = async (payload) => {
  try {
    return await apiRequest('/api/onboarding/parent_profile', {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  } catch (err) {
...
export const getOnboardingStatus = async () => {
  try {
    ...
    if (res?.error && res.error.status === 404) {
      return {
        data: { onboarding_completed: false, has_children: false, has_subjects: false, has_parent_profile: false, default_planning_mode: null },
        error: null,
      };
    }
