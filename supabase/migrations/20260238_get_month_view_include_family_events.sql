-- Include whole-family / multi-child events in month view when filtering by child
-- When _child_ids is set, show events where e.child_id = ANY(_child_ids) OR (e.child_id IS NULL AND e.child_ids && _child_ids).
-- Use family timezone for date_local so events appear on the correct calendar day (fixes events missing when TZ differs from default).

-- Ensure family timezone helper exists (idempotent)
...
BEGIN
  SELECT EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'family' AND column_name = 'timezone'
  ) INTO v_has;
  IF v_has THEN
    SELECT timezone INTO v_tz FROM family WHERE id = p_family_id;
  END IF;
  RETURN COALESCE(NULLIF(TRIM(v_tz), ''), NULLIF(v_tz, 'UTC'), 'America/New_York');
END;
$$;
...
-- Get events for the entire month (EXCLUDE CANCELED AND DELETED EVENTS)
-- Include events for selected child: child_id match OR whole-family (child_ids overlaps _child_ids)
events AS (
  SELECT
    e.id,
    e.child_id,
...
COMMENT ON FUNCTION get_month_view(UUID, INTEGER, INTEGER, UUID[]) IS 'Month calendar view. When _child_ids is set, includes events for that child and whole-family events (child_ids && _child_ids). Uses family timezone for correct date.';
