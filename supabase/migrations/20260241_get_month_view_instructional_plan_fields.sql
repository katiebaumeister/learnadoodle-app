-- Add counts_toward_plan, instructional_status, academic_year_id, child_ids to get_month_view
-- so planner export "Count as instructional time" and "Plan My Year" columns are correct,
-- and Child column can show all assignees for multi-child events.
...
COMMENT ON FUNCTION get_month_view(UUID, INTEGER, INTEGER, UUID[]) IS 'Month calendar view. Includes counts_toward_plan, instructional_status, academic_year_id, child_ids for export and UI.';
