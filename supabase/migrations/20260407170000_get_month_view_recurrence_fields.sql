-- Include recurrence fields in get_month_view so planner can detect series and show
-- "delete this occurrence" vs "delete all in series" (matches get_week_view).
...
COMMENT ON FUNCTION get_month_view(UUID, INTEGER, INTEGER, UUID[]) IS 'Month calendar view. Includes recurrence_rule, parent_event_id, recurrence_id for recurring delete UX.';
