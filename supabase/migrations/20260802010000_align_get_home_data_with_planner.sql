-- Align get_home_data with Planner day semantics (undo America/New_York regression
-- from 2025_clean_invalid_avatar_uuids). Home schedule must match get_month_view:
-- family timezone, soft-delete/cancel filters, multi-day span inclusion, backlog excluded
-- from learning, and planner fields (child_ids, is_flexible, all_day, event_type, start_ts).
...
today_events AS (
  ...
  WHERE e.family_id = _family_id
    ...
    -- Calendar schedule only (Planner excludes backlog from month/week views)
    AND COALESCE(e.is_backlog, false) = false
    AND (
...
big_events AS (
  ...
  WHERE e.family_id = _family_id
    ...
    -- Exclude soft-deleted and canceled events
    AND (e.deleted_at IS NULL)
    AND (e.canceled_at IS NULL)
  ORDER BY e.start_ts
  LIMIT 5
...
scheduled_minutes AS (
  ...
  WHERE family_id = _family_id
    ...
    -- Exclude soft-deleted and canceled events from scheduled minutes calculation
