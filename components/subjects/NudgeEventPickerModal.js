...
export default function NudgeEventPickerModal({
  visible = false,
  onClose,
  onSent,
  familyId,
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      const withSubjects = (data || []).map((event) => ({
        ...event,
        subject: event.subject_id ? subjectsMap[event.subject_id] || null : null,
      }));
      setEvents(withSubjects);
    } catch (err) {
      setEvents([]);
      setError(err?.message || 'Could not load upcoming events.');
    } finally {
      setLoading(false);
    }
  }, [familyId, childId]);
  ...
  const handleSelectEvent = useCallback(async (event) => {
    if (!event?.id || !familyId || !childId || sendingId) return;
    setSendingId(event.id);
    setError('');
    try {
      const title = String(event?.title || 'this assignment').trim() || 'this assignment';
      const assignmentId = await ensureLinkedAssignment({
        familyId,
        event,
        childId,
        subjectId: event?.subject_id || event?.subject?.id || null,
        userId: authUserId,
        title: event?.title,
        status: 'not_started',
      });
      if (!assignmentId) throw new Error('Could not link assignment');
      await appendAssignmentMessage(assignmentId, `Reminder: ${title}`, 'sent_assignment');
      toast.push(`Nudge sent to ${childName}`, 'success');
      dispatchAssignmentRefreshEvents();
      onSent?.();
      onClose?.();
    } catch (err) {
  ...
  }, [authUserId, childId, childName, familyId, onClose, onSent, sendingId, toast]);
      ...
      <View style={styles.overlay}>
        ...
        <View style={styles.sheet}>
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            <Text style={styles.title}>Which event?</Text>
            ...
            {!loading
              ? events.map((event) => {
                ...
                return (
                  <TouchableOpacity
                    key={event.id}
                    style={[styles.eventRow, isSending && styles.eventRowDisabled]}
                    onPress={() => handleSelectEvent(event)}
                    disabled={!!sendingId}
                    activeOpacity={0.8}
                    {...(Platform.OS === 'web' && { cursor: sendingId ? 'not-allowed' : 'pointer' })}
                  >
                    ...
                    {isSending ? <ActivityIndicator size="small" color="#64748B" /> : null}
                  </TouchableOpacity>
                );
              })
              : null}
