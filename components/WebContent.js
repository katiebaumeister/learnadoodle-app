...
I can see you have ${children.length} child(ren) set up. How can I help you today?`
      ...
      setDoodleMessages([...updatedMessages, { role: 'assistant', content: response.message, timestamp: Date.now() }])
  ...
  const handleSendMessage = () => {
    if (doodleInput.trim()) {
      handleDoodleMessage(doodleInput.trim())
      setDoodleInput('')
    }
  }
  ...
  const renderCalendarContent = () => {
    if (!familyId) {
        ...
        <CalendarView 
          ...
          onPlanNew={() => onTabChange('calendar-planning')}
        />
          </View>
        )
  }
