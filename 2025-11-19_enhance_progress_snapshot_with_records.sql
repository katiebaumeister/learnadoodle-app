-- Enhance get_progress_snapshot to include records data (grades, credits, portfolio)
-- This enables AI summaries to reference grades, credits, and portfolio evidence
...
AS $$
  SELECT 
    ...
    -- Latest grade for this child+subject in the date range
    (
      SELECT g.grade
      FROM grades g
      WHERE g.child_id = c.id
        AND g.subject_id = e.subject_id
        AND g.created_at::date BETWEEN p_start AND p_end
      ORDER BY g.created_at DESC
      LIMIT 1
    ) AS latest_grade,
    -- Total credits for this child+subject in the date range
    COALESCE((
      SELECT SUM(g.credits)
      FROM grades g
      WHERE g.child_id = c.id
        AND g.subject_id = e.subject_id
        AND g.created_at::date BETWEEN p_start AND p_end
    ), 0) AS credits,
    -- Portfolio upload count for this child+subject in the date range
    (
      SELECT COUNT(*)
      FROM uploads u
      WHERE u.child_id = c.id
        AND u.subject_id = e.subject_id
        AND u.created_at::date BETWEEN p_start AND p_end
    ) AS portfolio_count
  FROM events e
