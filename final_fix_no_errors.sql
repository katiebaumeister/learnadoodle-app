-- =====================================================
-- FINAL FIX - AVOID ALL TYPE CASTING ISSUES
-- Use dynamic SQL to avoid type conflicts
-- =====================================================
...
-- 2. Add goals for Lilly using dynamic SQL to avoid type issues
DO $$
DECLARE
...
BEGIN
  ...
  EXECUTE format('SELECT id::text FROM children WHERE family_id = %L::uuid AND first_name = %L', v_family_id, 'Lilly') INTO v_lilly_id;
  ...
  IF v_lilly_id IS NOT NULL THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'subject_goals') THEN
      ...
      -- Build dynamic SQL to avoid type casting issues
      IF v_has_goal_minutes THEN
        v_sql := format('
          INSERT INTO subject_goals (child_id, subject_id, goal_minutes_per_week)
          SELECT %L::uuid, s.id::uuid, 120
          FROM subject s
          WHERE s.family_id = %L::uuid
            AND NOT EXISTS (
              SELECT 1 FROM subject_goals sg
              WHERE sg.child_id = %L::uuid AND sg.subject_id = s.id::uuid
            )', v_lilly_id, v_family_id, v_lilly_id);
      ELSE
        v_sql := format('
          INSERT INTO subject_goals (child_id, subject_id)
          SELECT %L::uuid, s.id::uuid
          FROM subject s
          WHERE s.family_id = %L::uuid
            AND NOT EXISTS (
              SELECT 1 FROM subject_goals sg
...
BEGIN
  RAISE NOTICE '╔════════════════════════════════════════╗';
  RAISE NOTICE '║  FINAL FIX - NO MORE TYPE ERRORS       ║';
  RAISE NOTICE '╚════════════════════════════════════════╝';
  RAISE NOTICE '';
